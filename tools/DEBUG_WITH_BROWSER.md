...
|   CLI          |    localhost:9227  |  Server (tokio)  |                    |  (Manifest V3)   |
+----------------+                    +------------------+                    +------------------+
                                                                                      |
                                                                                      | DOM Access
                                                                                      v
...
```
Wait for compilation (~1-2 minutes). Server runs on http://localhost:8086

### Terminal 2: Start the WebSocket Server with Hot Reload
```bash
cd tools && cargo run --release -- server start --port 9227 --watch ./extension
```
Server listens on ws://127.0.0.1:9227 and watches for extension file changes.
...
5. Navigate to http://localhost:8086
...
boon-tools server start --port 9227

# Start with extension hot reload (watches for file changes)
boon-tools server start --port 9227 --watch ./extension
```
...
│   (CLI/Rust)    │     port 9227      │  (background.js)    │  chrome.debugger │   (Tab)     │
└─────────────────┘                    └─────────────────────┘                  └─────────────┘
```
...
```
WebSocket server listening on ws://127.0.0.1:9227
...
1. **Setup (once)**:
   ...
   chromium --load-extension=./extension --user-data-dir=/tmp/boon-chromium \
            --no-first-run http://localhost:8086
   ```
