...
class Gate {
    constructor(id, graph, gateParams, ports, inputSignals, outputSignals) {
        ...
        // The DFF cell sets this in initialize() which is not called for worker Gates
        // IMPORTANT: Must be -1 (Vector3vl LOW) not 0, so first rising edge is detected
        // Vector3vl.get() returns: -1=LOW, 0=X, 1=HIGH
        // See: DEBUGGING.md for details on this bug discovery
...
class WorkerEngineWorker {
    ...
    // DEBUG: dump gate state
    debugGate(graphId, gateId, reqid) {
        const graph = this._graphs[graphId];
        if (!graph) {
            postMessage({ type: 'debug', args: [reqid, { error: 'graph not found' }] });
            return;
        }
        const gate = graph.getGate(gateId);
        if (!gate) {
            postMessage({ type: 'debug', args: [reqid, { error: 'gate not found' }] });
            return;
        }
        const targets = gate._links_to || {};
        const targetCounts = {};
        for (const [port, set] of Object.entries(targets)) {
            targetCounts[port] = set ? set.size : 0;
        }

        // For subcircuits, also check internal Input gates
        let subgraphInfo = null;
        if (gate.isSubcircuit) {
            const subgraph = gate.get('subgraph');
            const iomap = gate.get('circuitIOmap');
            if (subgraph && iomap) {
                subgraphInfo = {
                    graphId: subgraph.id,
                    gateCount: subgraph._gates ? Object.keys(subgraph._gates).length : 0,
                    linkCount: subgraph._links ? Object.keys(subgraph._links).length : 0,
                    inputGates: {}
                };
                // Check each input port's internal gate
                for (const [port, internalGateId] of Object.entries(iomap)) {
                    const internalGate = subgraph.getGate(internalGateId);
                    if (internalGate) {
                        const internalTargets = internalGate._links_to || {};
                        const internalTargetCounts = {};
                        for (const [p, s] of Object.entries(internalTargets)) {
                            internalTargetCounts[p] = s ? s.size : 0;
                        }
                        subgraphInfo.inputGates[port] = {
                            id: internalGateId,
                            type: internalGate.get('type'),
                            targetCounts: internalTargetCounts
                        };
                    } else {
                        subgraphInfo.inputGates[port] = { id: internalGateId, error: 'not found' };
                    }
                }
            }
        }

        postMessage({ type: 'debug', args: [reqid, {
            id: gate.id,
            isSubcircuit: gate.isSubcircuit,
            hasSubgraph: !!gate.get('subgraph'),
            inputPorts: Object.keys(gate.get('inputSignals') || {}),
            outputPorts: Object.keys(gate.get('outputSignals') || {}),
            targetCounts,
            subgraphInfo
        }] });
    }
    ...
    _setGateInputSignal(targetGate, port, sig) {
        ...
        if (targetGate.isSubcircuit) {
            ...
            if (!subgraph) {
                console.log('[WORKER DEBUG] Subcircuit has no subgraph:', targetGate.id, port);
                return;
            }
            const iomap = targetGate.get('circuitIOmap');
            const internalGateId = iomap[port];
            const gate = subgraph.getGate(internalGateId);
            if (!gate) {
                console.log('[WORKER DEBUG] Internal gate not found:', targetGate.id, port, internalGateId);
                return;
            }
