...
export class SynchEngine extends BaseEngine {
    constructor(graph, {cells}) {
        ...
        this._recursiveSubcircuits = true;  // Enable subcircuit simulation
    ...
    _updateSubcircuit(gate, sigs, prevSigs = {}) {
        ...
        for (const [port, sig] of Object.entries(sigs)) {
            ...
            console.log(`[SynchEngine] _updateSubcircuit ${label}.${port} = ${sig.toBin ? sig.toBin() : sig}`);
            input._setInput(sig);
        }
    }
    _addGate(graph, gate) {
