...
var MountedTree = class {
	constructor(tree, overlay, parser$2) {
		this.tree = tree;
...
function defineLanguageFacet(baseData) {
	return Facet.define({ combine: baseData ? (values) => values.concat(baseData) : undefined });
}
/**
...
var Language = class {
	...
	*/
	constructor(data, parser$2, extraExtensions = [], name$1 = "") {
		this.data = data;
		this.name = name$1;
...
var LRLanguage = class LRLanguage extends Language {
	constructor(data, parser$2, name$1) {
		super(data, parser$2, [], name$1);
		this.parser = parser$2;
	}
	/**
	...
	*/
	static define(spec) {
		...
		return new LRLanguage(data, spec.parser.configure({ props: [languageDataProp.add((type) => type.isTop ? data : undefined)] }), spec.name);
	}
	/**
...
var ParseContext = class ParseContext {
	constructor(parser$2, state, fragments = [], tree, treeLen, viewport, skipped, scheduleOn) {
		this.parser = parser$2;
		this.state = state;
	...
	@internal
	*/
	static create(parser$2, state, viewport) {
		return new ParseContext(parser$2, state, [], Tree.empty, 0, viewport, [], null);
	}
	startParse() {
...
var Stack = class Stack {
	...
	*/
	reduce(action) {
		...
		let { parser: parser$2 } = this.p;
		let lookaheadRecord = this.reducePos < this.pos - 25;
		if (lookaheadRecord) this.setLookAhead(this.pos);
		let dPrec = parser$2.dynamicPrecedence(type);
		if (dPrec) this.score += dPrec;
		if (depth == 0) {
			this.pushState(parser$2.getGoto(this.state, type, true), this.reducePos);
			if (type < parser$2.minRepeatTerm) this.storeNode(type, this.reducePos, this.reducePos, lookaheadRecord ? 8 : 4, true);
		...
		if (type < parser$2.minRepeatTerm || action & 131072) {
			let pos = parser$2.stateFlag(
				this.state,
				1
				/* StateFlag.Skipped */
		...
		if (action & 262144) this.state = this.stack[base$1];
			...
			this.state = parser$2.getGoto(baseStateID, type, true);
		}
	...
	*/
	shift(action, type, start, end) {
		if (action & 131072) this.pushState(action & 65535, this.pos);
			...
			let nextState = action, { parser: parser$2 } = this.p;
			if (end > this.pos || type <= parser$2.maxNode) {
				this.pos = end;
				if (!parser$2.stateFlag(
					nextState,
					1
					/* StateFlag.Skipped */
)) this.reducePos = end;
			...
			if (type <= parser$2.maxNode) this.buffer.push(type, start, end, 4);
		} else {
			this.pos = end;
			this.shiftContext(type, start);
	...
	*/
	forceReduce() {
		let { parser: parser$2 } = this.p;
		let reduce = parser$2.stateSlot(
			this.state,
			5
			/* ParseState.ForcedReduce */
);
		if ((reduce & 65536) == 0) return false;
		if (!parser$2.validAction(this.state, reduce)) {
			let depth = reduce >> 19, term = reduce & 65535;
			let target = this.stack.length - depth * 3;
			if (target < 0 || parser$2.getGoto(this.stack[target], term, false) < 0) {
	...
	*/
	findForcedReduction() {
		let { parser: parser$2 } = this.p, seen = [];
		let explore = (state, depth) => {
			if (seen.includes(state)) return;
			seen.push(state);
			return parser$2.allActions(state, (action) => {
				if (action & 393216);
				else if (action & 65536) {
					let rDepth = (action >> 19) - depth;
					if (rDepth > 1) {
						let term = action & 65535, target = this.stack.length - rDepth * 3;
						if (target >= 0 && parser$2.getGoto(this.stack[target], term, false) >= 0) return rDepth << 19 | 65536 | term;
					}
				} else {
	...
	*/
	get deadEnd() {
		...
		let { parser: parser$2 } = this.p;
		return parser$2.data[parser$2.stateSlot(
			this.state,
			1
			/* ParseState.Actions */
)] == 65535 && !parser$2.stateSlot(
			this.state,
			4
			/* ParseState.DefaultReduce */
);
...
var TokenGroup = class {
	...
	token(input, stack) {
		let { parser: parser$2 } = stack.p;
		readToken(this.data, input, stack, this.id, parser$2.data, parser$2.tokenPrecTable);
	}
};
...
function cutAt(tree, pos, side) {
	let cursor$1 = tree.cursor(IterMode.IncludeAnonymous);
	cursor$1.moveTo(pos);
	for (;;) if (!(side < 0 ? cursor$1.childBefore(pos) : cursor$1.childAfter(pos))) for (;;) {
		if ((side < 0 ? cursor$1.to < pos : cursor$1.from > pos) && !cursor$1.type.isError) return side < 0 ? Math.max(0, Math.min(
			cursor$1.to - 1,
			pos - 25
			/* Lookahead.Margin */
		...
		if (side < 0 ? cursor$1.prevSibling() : cursor$1.nextSibling()) break;
		if (!cursor$1.parent()) return side < 0 ? 0 : tree.length;
	}
}
...
var TokenCache = class {
	constructor(parser$2, stream) {
		this.stream = stream;
		this.tokens = [];
		this.mainToken = null;
		this.actions = [];
		this.tokens = parser$2.tokenizers.map((_) => new CachedToken());
	}
	getActions(stack) {
		let actionIndex = 0;
		let main = null;
		let { parser: parser$2 } = stack.p, { tokenizers } = parser$2;
		let mask = parser$2.stateSlot(
			stack.state,
			3
	...
	updateCachedToken(token, tokenizer, stack) {
		...
		if (token.value > -1) {
			let { parser: parser$2 } = stack.p;
			for (let i$1 = 0; i$1 < parser$2.specialized.length; i$1++) if (parser$2.specialized[i$1] == token.value) {
				let result = parser$2.specializers[i$1](this.stream.read(token.start, token.end), stack);
	...
	addActions(stack, token, end, index) {
		let { state } = stack, { parser: parser$2 } = stack.p, { data } = parser$2;
		for (let set = 0; set < 2; set++) for (let i$1 = parser$2.stateSlot(
			state,
			set ? 2 : 1
			/* ParseState.Actions */
);; i$1 += 3) {
...
var Parse = class {
	constructor(parser$2, input, fragments, ranges) {
		this.parser = parser$2;
		this.input = input;
		this.ranges = ranges;
		this.recovering = 0;
		...
		this.tokens = new TokenCache(parser$2, this.stream);
		this.topTerm = parser$2.top[1];
		let { from } = ranges[0];
		this.stacks = [Stack.start(this, parser$2.top[0], from)];
		this.fragments = fragments.length && this.stream.end - from > parser$2.bufferLength * 4 ? new FragmentCursor(fragments, parser$2.nodeSet) : null;
	}
	get parsedPos() {
	...
	advanceStack(stack, stacks, split) {
		let start = stack.pos, { parser: parser$2 } = this;
		let base$1 = verbose ? this.stackID(stack) + " -> " : "";
		if (this.stoppedAt != null && start > this.stoppedAt) return stack.forceReduce() ? stack : null;
		if (this.fragments) {
			let strictCx = stack.curContext && stack.curContext.tracker.strict, cxHash = strictCx ? stack.curContext.hash : 0;
			for (let cached = this.fragments.nodeAt(start); cached;) {
				let match = this.parser.nodeSet.types[cached.type.id] == cached.type ? parser$2.getGoto(stack.state, cached.type.id) : -1;
				if (match > -1 && cached.length && (!strictCx || (cached.prop(NodeProp.contextHash) || 0) == cxHash)) {
					stack.useNode(cached, match);
					if (verbose) console.log(base$1 + this.stackID(stack) + ` (via reuse of ${parser$2.getName(cached.type.id)})`);
					return true;
				}
		...
		let defaultReduce = parser$2.stateSlot(
			stack.state,
			4
			/* ParseState.DefaultReduce */
);
		if (defaultReduce > 0) {
			...
			if (verbose) console.log(base$1 + this.stackID(stack) + ` (via always-reduce ${parser$2.getName(
				defaultReduce & 65535
				/* Action.ValueMask */
)})`);
			return true;
		...
		for (let i$1 = 0; i$1 < actions.length;) {
			...
			if (verbose) console.log(base$1 + this.stackID(localStack) + ` (via ${(action & 65536) == 0 ? "shift" : `reduce of ${parser$2.getName(
				action & 65535
				/* Action.ValueMask */
)}`} for ${parser$2.getName(term)} @ ${start}${localStack == stack ? "" : ", split"})`);
			if (last) return true;
...
var LRParser = class LRParser extends Parser {
	...
	constructor(spec) {
		...
		*/
		...
		this.nodeSet = new NodeSet(nodeNames.map((name$1, i$1) => NodeType.define({
			name: i$1 >= this.minRepeatTerm ? undefined : name$1,
			id: i$1,
			props: nodeProps[i$1],
	...
	*/
	hasAction(state, terminal) {
		let data = this.data;
		for (let set = 0; set < 2; set++) for (let i$1 = this.stateSlot(
			state,
			set ? 2 : 1
			/* ParseState.Actions */
), next;; i$1 += 3) {
	...
	*/
	parseDialect(dialect) {
		...
		let disabled = null;
		for (let i$1 = 0; i$1 < values.length; i$1++) if (!flags[i$1]) for (let j = this.dialects[values[i$1]], id; (id = this.data[j++]) != 65535;) (disabled || (disabled = new Uint8Array(this.maxTerm + 1)))[id] = 1;
...
const parser$1 = LRParser.deserialize({
	version: 14,
	states: "@|OYQPOOOOQO'#EY'#EYOYQPO'#C`O#zQPO'#CoOOQO'#DW'#DWO$[QPO'#C|O%qQPO'#D[O%xQPO'#D]O%}QPO'#D_O'aQPO'#DcO'kQPO'#DhO'kQPO'#DiO'pQPO'#DjO'uQPO'#DoO'kQPO'#DpO'zQPO'#DqOOQO'#Dr'#DrOOQO'#Ds'#DsO)bQPO'#DWO*oQPO'#C{O$dQPO'#CxOOQO'#Cx'#CxO,SQPO'#CwO-dQPO'#CvO.qQPO'#CuO0ZQPO'#CtO0wQPO'#CsOOQO'#Cr'#CrO1cQPO'#CcOOQO'#Cb'#CbO1hQPO'#C`QOQPOOOOQO-E8W-E8WO1pQPO,58zO1xQPO'#E[O1}QPO,59ZOOQO,59i,59iO3hQPO'#C}O$[QPO'#DQOOQO'#DQ'#DQOOQO,59h,59hOOQO,59v,59vO3mQPO,59vO3rQPO,59wO3|QPO,59yO4bQPO'#CoOOQO'#Dg'#DgO4iQPO'#D
fT!ih!jT!hh!jT!mi!nT!li!nT!pj!qR#`!UQQOYpQ!s!u#{$oQ!snS!uq!tR#{!vQ%d$mR&S%dSsR}S!xs%UR%U$`Q!_cR#m!_Q$h#hR%`$hQ%b$iR&Q%bQ#}!{R$r#}Q$t$PR%j$tQ$S#SR$y$SQ\${$UR%o\${Q#Y!PR$Y#YQ$[#[R%R$[Q%X$dQ%[$eT%z%X%[Q!fgR#p!fQ!jhR#r!jQ!niR#t!nQ!qjR#v!qQ%T$^R%t%TQ!tnQ!vqT#z!t!v",
	nodeNames: "⚠ WS LineComment Program ProgramItems Newline Item FunctionDef SnakeCaseIdentifier ParamList BracketRoundOpen ParamEntry Comma BracketRoundClose Block BracketCurlyOpen BracketCurlyClose Statement Definition Name Dot Colon Expression PipeExpr CompareExpr AddExpr MulExpr PowExpr UnaryExpr Minus Plus PostfixExpr ModulePathLiteral ModulePathStart PascalCaseIdentifier Slash ModulePathTail CallSuffix CallArguments NamedArgumentList NamedArgument MultilineArguments PrimaryBase Number Text Wildcard Parenthesized ListLiteral ListCommaEntries MapLiteral MapCommaEntries MapEntry MapKey ObjectLiteral BracketSquareOpen BracketSquareClose ObjectCommaEntries ObjectEntry LatestExpr ThenExpr WhenExpr WhenArm WhenPattern Implies WhenBody WhileExpr BlockExpr LinkExpr SkipLiteral PassLiteral PowTail Caret MulTail MulOperator Asterisk Percent AddTail AddOperator CompareTail CompareOperator Equal NotEqual GreaterOrEqual LessOrEqual Greater Less PipeTail Pipe PipeBreak BlockLines",
	maxTerm: 121,
	skippedNodes: [
		0,
		1,
	...
	repeatNodeCount: 19,
	tokenData: "2v~RzXY#uYZ$T]^#upq#uuv%Qwx%Vxy&yyz'Oz{'T{|'Y|}'_}!O'd!O!P(j!P!Q(o!Q![(t![!])_!^!_)d!_!`)q!`!a*^!c!d*k!d!e+P!e!h*k!h!i+q!i!n*k!n!o,u!o!p.a!p!r*k!r!s.u!s!u*k!u!v/a!v!w/{!w!y*k!y!z0g!z!}*k!}#O1g#P#Q1l#Q#R1q#R#S1v#T#o2R#o#p2a#p#q2f#q#r2q~#zRP~XY#u]^#upq#u~$YST~XY$f]^$fpq$f#p#q$u~$iSXY$f]^$fpq$f#p#q$u~$xP!`!a\${~%QO!z~~%VO!m~~%YWOY%VZw%Vwx%rx#O%V#O#P%w#P;'S%V;'S;=`&s<%lO%V~%wO|~~%zRO;'S%V;'S;=`&T;=`O%V~&WXOY%VZw%Vwx%rx#O%V#O#P%w#P;'S%V;'S;=`&s;=`<%l%V<%lO%V~&vP;=`<%l%V~'OOY~~'TO]~~'YO!l~~'_On~~'dO[~~'gTO}'v}!O'{!O;'S'v;'S;=`(d<%lO'v~'{Om~~(QSQ~OY'{Z;'S'{;'S;=`(^<%lO'{~(aP;=`<%l'{~(gP;=`<%l'v~(oOd~~(tOs~~(yQ{~!O!P)P!Q![(t~)SP!Q![)V~)[P{~!Q![)V~)dOe~~)iP!w~!_!`)l~)qO!u~~)vQ!rP!P!Q)|!`!a*X~*PP!_!`*S~*XO!s~Q*^O!aQ~*cP!v~!_!`*f~*kO!t~~*nP#T#o*q~*vRr~!Q![*q!c!}*q#T#o*q~+SQ!n!o+Y#T#o*q~+]P!q!r+`~+cP!e!f+f~+iP!m!n+l~+qO#j~~+tQ!w!x+z#T#o*q~+}P!p!q,Q~,TP!e!f,W~,ZP!v!w,^~,aP!k!l,d~,gP!q!r,j~,mP!p!q,p~,uO#c~~,xR!c!d-R!k!l-p#T#o*q~-UP!v!w-X~-[P!g!h-_~-bP!u!v-e~-hP!v!w-k~-pO#f~~-sQ!p!q-y!u!v.U~-|P!m!n.P~.UO#k~~.XP!v!w.[~.aO#d~~.dQ!c!d.j#T#o*q~.mP!r!s.p~.uO#e~~.xQ!c!d/O#T#o*q~/RP!u!v/U~/XP!u!v/[~/aO#m~~/dQ!m!n/j#T#o*q~/mP!k!l/p~/sP!r!s/v~/{O#l~~0OQ!j!k0U#T#o*q~0XP!g!h0[~0_P!p!q0b~0gO#g~~0jQ!j!k0p#T#o*q~0sQ!g!h0y!k!l1U~0|P!p!q1P~1UO#h~~1XP!n!o1[~1_P!g!h1b~1gO#i~~1lO!W~~1qO!X~~1vO!i~~1yP#R#S1|~2RO}~~2WRW~!Q![2R#R#S2R#T#o2R~2fO_~~2iP!`!a2l~2qO!y~~2vO`~",
	tokenizers: [0, 1],
...
//#endregion
//#region boon-language.ts
const parser = parser$1.configure({ props: [styleTags({
	SnakeCaseIdentifier: tags.variableName,
	ModulePathLiteral: tags.namespace,
	ModulePathTail: tags.namespace,
	ModulePathStart: tags.namespace,
	"ModulePathLiteral/ModulePathStart/PascalCaseIdentifier": tags.namespace,
	"ModulePathTail/ModulePathStart/PascalCaseIdentifier": tags.namespace,
	"ModulePathTail/SnakeCaseIdentifier": tags.namespace,
	"ModulePathLiteral/ModulePathStart/Slash": tags.separator,
	"ModulePathTail/ModulePathStart/Slash": tags.separator,
	PascalCaseIdentifier: tags.typeName,
	Wildcard: tags.special(tags.variableName),
	Number: tags.number,
	Text: tags.string,
	LineComment: tags.lineComment,
	Pipe: tags.operator,
	PipeBreak: tags.operator,
	AddOperator: tags.operator,
	MulOperator: tags.operator,
	Caret: tags.operator,
	Percent: tags.operator,
	CompareOperator: tags.operator,
	Colon: tags.separator,
	Comma: tags.separator,
	Dot: tags.separator,
	BracketRoundOpen: tags.paren,
	...
	"FUNCTION THEN WHEN WHILE LATEST BLOCK LINK SKIP PASS LIST MAP": tags.keyword,
	"FunctionDef/SnakeCaseIdentifier": tags.definition(tags.function(tags.variableName)),
	"ParamEntry/SnakeCaseIdentifier": tags.local(tags.variableName),
	"NamedArgument/SnakeCaseIdentifier": tags.propertyName,
	"Definition/Name/SnakeCaseIdentifier": tags.definition(tags.variableName)
})] });
