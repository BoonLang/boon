...
impl LoweredProgram {
    pub fn into_host_view(self) -> Result<HostViewIr, String> {
        match self {
            ...
            Self::TodoMvc(program) => Ok(program.host_view),
    ...
    pub fn into_counter_program(self) -> Result<CounterProgram, String> {
        match self {
            ...
            Self::TodoMvc(_) => Err("generic lowerer matched todo_mvc, not counter".to_string()),
    ...
    pub fn into_todo_program(self) -> Result<TodoProgram, String> {
        match self {
            Self::TodoMvc(program) => Ok(program),
    ...
    pub fn into_latest_program(self) -> Result<LatestProgram, String> {
        match self {
            ...
            Self::TodoMvc(_) => Err("generic lowerer matched todo_mvc, not latest".to_string()),
    ...
    pub fn into_then_program(self) -> Result<ThenProgram, String> {
        match self {
            ...
            Self::TodoMvc(_) => Err("generic lowerer matched todo_mvc, not then".to_string()),
    ...
    pub fn into_when_program(self) -> Result<WhenProgram, String> {
        match self {
            ...
            Self::TodoMvc(_) => Err("generic lowerer matched todo_mvc, not when".to_string()),
    ...
    pub fn into_while_program(self) -> Result<WhileProgram, String> {
        match self {
            ...
            Self::TodoMvc(_) => Err("generic lowerer matched todo_mvc, not while".to_string()),
...
fn lowered_program_subset(program: &LoweredProgram) -> &'static str {
    match program {
        ...
        LoweredProgram::TodoMvc(_) => "editable_filterable_list_document",
...
fn editable_filterable_list_selected_filter(
    ...
    match base_sink_values.get(&TodoProgram::SELECTED_FILTER_SINK) {
        Some(KernelValue::Text(text)) | Some(KernelValue::Tag(text)) if text == "active" => {
            TodoSelectedFilter::Active
        }
        Some(KernelValue::Text(text)) | Some(KernelValue::Tag(text)) if text == "completed" => {
...
fn lower_append_list_program_surface_owned(
    ...
    let (ir, host_view) = lower_append_list_surface_owned(expressions, config.surface)?;
    Ok(build_ir_host_view_lowered_program(
        ir,
        host_view,
        config.program,
...
fn append_timer_source_child(
    ...
    let view_site = ViewSiteId(next_free_view_site(host_view));
...
fn lower_timer_backed_signal_host_view<'a>(
    ...
    append_timer_source_child(&mut host_view, function_instance, tick_port, interval_ms);
    Ok(host_view)
}
...
fn try_lower_todo_mvc_from_expressions(
    ...
    lower_bindings_group_typed_program(
        ...
        |program| match program {
            LoweredProgram::TodoMvc(program) => Some(program),
            _ => None,
        },
    )
}
