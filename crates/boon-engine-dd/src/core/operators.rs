...
where
    G: Scope<Timestamp = u64>,
{
    let mut remaining_to_skip = count;
    let mut skipped_values: BTreeMap<Value, isize> = BTreeMap::new();
        ...
        .unary::<CapacityContainerBuilder<Vec<_>>, _, _, _>(Pipeline, "Skip", |_cap, _info| {
            move |input, output| {
                input.for_each(|time, data| {
                    ...
                    for (value, ts, diff) in data.drain(..) {
                        if diff > 0 {
                            let mut emit_diff = diff;
                            while remaining_to_skip > 0 && emit_diff > 0 {
                                *skipped_values.entry(value.clone()).or_default() += 1;
                                remaining_to_skip -= 1;
                                emit_diff -= 1;
                            }
                            if emit_diff > 0 {
                                session.give((value, ts, emit_diff));
                            }
                        } else {
                            let mut remaining_retraction = -diff;
                            let mut remove_entry = false;
                            if let Some(skipped_count) = skipped_values.get_mut(&value) {
                                let absorbed = (*skipped_count).min(remaining_retraction);
                                *skipped_count -= absorbed;
                                remaining_retraction -= absorbed;
                                remove_entry = *skipped_count == 0;
                            }
                            if remove_entry {
                                skipped_values.remove(&value);
                            }
                            if remaining_retraction > 0 {
                                session.give((value, ts, -remaining_retraction));
                            }
                        }
                    }
                });
