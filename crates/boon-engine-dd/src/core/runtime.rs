    ...
    while !pending.is_empty() {
        ...
        for (var_id, spec) in pending {
            ...
            if std::env::var_os("BOON_DD_TRACE_CRUD").is_some()
                || std::env::var_os("BOON_DD_TRACE_SHOPPING").is_some()
            {
                let name = var_id.as_str().to_string();
                let trace_crud = std::env::var_os("BOON_DD_TRACE_CRUD").is_some()
                    && (name == "store.person_to_add"
                        || name == "store.people"
                        || name.contains("append_keyed")
                        || name.contains("membership")
                        || name.contains("keyed_hold")
                        || name.contains("assemble_raw"));
                let trace_shopping = std::env::var_os("BOON_DD_TRACE_SHOPPING").is_some()
                    && (name == "store.text_to_add"
                        || name == "store.items"
                        || name == "store.elements.item_input.text"
                        || name.contains("text_key_state")
                        || name.contains("append_event")
                        || name.contains("list_events"));
                let should_trace = trace_crud || trace_shopping;
                if should_trace {
