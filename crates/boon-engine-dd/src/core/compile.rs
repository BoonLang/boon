...
use boon::parser::{
    Parser as _, SourceCode, lexer, parser, reset_expression_depth, resolve_references, span_at,
    static_expression::{
...
pub fn compile(
    ...
    let trace_cells = std::env::var_os("BOON_DD_TRACE_CELLS").is_some()
        && source_code.contains("-- 7GUIs Task 7: Cells (Spreadsheet)");
    if trace_cells {
...
impl Compiler {
    ...
    fn resolve_scope_alias_base_path(
        ...
        let root_name = parts.first()?.as_str();
        let mut value = scope.get(root_name).cloned()?;
        for part in &parts[1..] {
            value = value.get_field(part.as_str()).cloned()?;
        }
        Self::reactive_value_base_path_in_scope(&value)
    }

    fn reactive_value_base_path_in_scope(value: &Value) -> Option<String> {
        match value {
            Value::Tagged { tag, fields } if tag.as_ref() == LIST_TAG => {
                let (first_key, first_item) = fields.iter().next()?;
                let item_path = Self::reactive_value_base_path_in_scope(first_item)?;
                item_path
                    .strip_suffix(&format!(".{}", first_key.as_ref()))
                    .map(|path| path.to_string())
                    .or_else(|| item_path.rsplit_once('.').map(|(path, _)| path.to_string()))
            }
            Value::Object(_) | Value::Tagged { .. } => value
                .get_field(LINK_PATH_FIELD)
                .and_then(|v| v.as_text())
                .map(|path| path.to_string())
                .or_else(|| {
                    value
                        .get_field(HOVER_PATH_FIELD)
                        .and_then(|v| v.as_text())
                        .map(|path| path.strip_suffix(".hovered").unwrap_or(path).to_string())
                }),
            _ => None,
        }
    }

    fn scope_root_path(scope: &IndexMap<String, Value>) -> Option<String> {
        scope
            .values()
            .filter_map(Self::reactive_value_base_path_in_scope)
        ...
        match &expr.node {
            ...
            Expression::Block {
                ...
                for var in variables {
                    let mut val = self.eval_static_with_scope(&var.node.value, &new_scope)?;
                    if self.expr_contains_link(&var.node.value)
                        && value_needs_reactive_decoration(&val)
                        && let Some(base_path) = Self::scope_root_path(local_scope)
                    {
                        val = decorate_reactive_scope_value(
                            &val,
                            &format!("{base_path}.{}", var.node.name.as_str()),
                        );
                    }
    ...
    fn eval_static_tolerant(
        ...
        match &expr.node {
            ...
            Expression::Block {
                ...
                let mut new_scope = local_scope.clone();
                for var in variables {
                    let mut val = self.eval_static_tolerant(&var.node.value, &new_scope);
                    if self.expr_contains_link(&var.node.value)
                        && value_needs_reactive_decoration(&val)
                        && let Some(base_path) = Self::scope_root_path(local_scope)
                    {
            ...
            Expression::Pipe { from, to } => {
                ...
                if let Expression::Hold { state_param, body } = &to.node {
                    ...
                    match &to.node {
                                ...
                                ["List", "map"] => {
                                    ...
                                    if let Some(expr) = map_expr {
                                        ...
                                        let source_base_path =
                                            Self::resolve_scope_alias_base_path(from, local_scope);
                                        if let Value::Tagged { tag, fields } = &from_val {
                                            if tag.as_ref() == LIST_TAG {
                                                let mapped_fields: BTreeMap<Arc<str>, Value> = fields
                                                    .iter()
                                                    .map(|(key, item_val)| {
                                                        ...
                                                        let mapped =
                                                            self.eval_static_tolerant(
                                                                &expr,
                                                                &map_scope,
                                                            );
                                                        let mapped =
                                                            if let Some(source_base_path) =
                                                                source_base_path.as_deref()
                                                            {
                        ...
                        Expression::LinkSetter { alias: _alias } => {
                            ...
                            if let Value::Tagged { tag, fields } = &from_val {
                                if tag.as_ref() == LIST_TAG {
                                    let mapped_fields: BTreeMap<Arc<str>, Value> = fields
                                        .iter()
                                        .map(|(key, item_val)| {
                                            let decorated_item = if let Some(source_base_path) =
                                                source_base_path.as_deref()
                                            {
                                            ...
                                            let mapped_item = if let Some(source_base_path) =
                                                source_base_path.as_deref()
                                            {
                                                decorate_reactive_scope_value_preserving_paths(
                                                    &mapped_item,
...
#[derive(Clone)]
struct StaticLinkForwarder {
    target_path: String,
    kind: InputKind,
    link_path: String,
    event_scope_path: String,
...
impl<'a> GraphBuilder<'a> {
    ...
    fn compile_program(
        ...
        let trace_compile = std::env::var_os("BOON_DD_TRACE_COMPILE").is_some();
        let trace_cells = std::env::var_os("BOON_DD_TRACE_CELLS").is_some()
            && self
                .compiler
                ...
                .unwrap_or(false);
        // Temporary diagnostic toggle: if CRUD stops stack-overflowing without
        // static link forwarders, the remaining regression is in that pass.
        // Revert once narrowed.
        if trace_compile {
            eprintln!("[dd-trace] before compile_static_link_forwarders");
        }
        self.compile_static_link_forwarders(doc_expr)?;
        if trace_compile {
            eprintln!("[dd-trace] after compile_static_link_forwarders");
        }
        ...
        while !pending.is_empty() {
            ...
            for (name, expr) in pending {
                if trace_compile {
                    eprintln!("[dd-trace] compile reactive var {name}");
                }
                if trace_cells {
        ...
        self.scope_prefix = None;
        if trace_compile {
            eprintln!("[dd-trace] after reactive pass1");
        }
        ...
        if trace_compile {
            eprintln!("[dd-trace] after extract_initial_passed");
        }
        ...
        let keyed_list_output = if let Some((list_name, keyed_var)) = self.keyed_hold_vars.first() {
            if trace_compile {
                eprintln!("[dd-trace] before build_display_pipeline {list_name}");
            }
            if trace_cells {
            ...
            if trace_compile {
                eprintln!("[dd-trace] after build_display_pipeline");
            }
        ...
        if trace_compile {
            eprintln!("[dd-trace] before compile_document_expr");
        }
        if trace_cells {
        ...
        if trace_compile {
            eprintln!("[dd-trace] after compile_document_expr");
        }
        if trace_cells {
    ...
    fn compile_reactive_var(
        ...
        match &expr.node {
            Expression::Link => {
                if let Some(existing) = self.reactive_vars.get(name) {
                    return Ok(existing.clone());
                }
                let var = VarId::new(name);
                if !self.collections.contains_key(&var) {
                    let input_id = self.add_input(InputKind::LinkPress, Some(name.to_string()));
                    self.collections
            ...
            Expression::Alias(Alias::WithoutPassed { parts, .. }) => {
                ...
                if let Some(ref prefix) = self.scope_prefix {
                    ...
                    if let Some(prefixed_expr) = self.compiler.get_var_expr(&prefixed) {
                        if self.is_collection_backed_reactive_expr(prefixed_expr) {
                            return Ok(VarId::new(prefixed.as_str()));
                        }
                    }
                }
                ...
                if let Some(var_expr) = self.compiler.get_var_expr(&path) {
                    if self.is_collection_backed_reactive_expr(var_expr) {
                        return Ok(VarId::new(path.as_str()));
                    }
                }
    ...
    fn compile_reactive_pipe(
        ...
        match &to.node {
            ...
            Expression::Then { body } => {
                ...
                let mut reactive_deps = self.find_sampled_reactive_deps_in_expr(body);
                ...
                #[cfg(test)]
                if name == "store.person_to_add" {
                    eprintln!(
                        "[dd-then-deps-debug] name={} event={:?} deps={:?}",
                        name, event_source_name, reactive_deps
                    );
                }
    ...
    fn compile_hold(
        ...
        let (events_var, _link_path) = self.compile_hold_body_events(name, state_param, body)?;

        // Build the transform closure
        let transform = self.build_hold_transform(name, state_param, body);
            ...
            Expression::Pipe { from, to } => {
                match &to.node {
                    Expression::Then { body: then_body } => {
                        ...
                        if depends_on_state {
                            let hold_marker = hold_name.to_string();
                            let marker_var =
                                self.fresh_var(&format!("{}_stateful_then", hold_name));
                            self.collections.insert(
                                marker_var.clone(),
                                CollectionSpec::Then {
                                    source: source_var,
                            ...
                            return Ok((marker_var, link_path));
                        }
                        let uses_event_alias = event_source_name
                            .as_ref()
    ...
    fn compile_event_source(
        ...
        match &expr.node {
            Expression::Alias(alias) => {
                ...
                let (path_str, var_name, is_passed_alias) = match alias {
                    Alias::WithoutPassed { parts, .. } => (
                        parts
                            .iter()
                            .map(|p| p.as_str())
                    ...
                    Alias::WithPassed { extra_parts } => (
                        ...
                        extra_parts
                            ...
                            .unwrap_or_else(|| PASSED_VAR.to_string()),
                        true,
                    ),
                };
                ...
                if !path_str.contains('.') {
                    if let Some(var_id) = self.reactive_vars.get(var_name.as_str()) {
                        return Ok((var_id.clone(), None));
                    }
                    if let Some(var_expr) = self.compiler.get_var_expr(var_name.as_str()) {
                        if let Some(target_path) = alias_target_path(var_expr)
                            && target_path != path_str
                            && let Ok(resolved) = self.compile_event_source(var_expr)
                        {
                            return Ok(resolved);
                        }
                        if self.compiler.is_reactive(var_expr) {
                            return Ok((VarId::new(var_name.as_str()), None));
                        }
                    }
                }
                ...
                let effective_path = if let Some(ref prefix) = self.scope_prefix {
                    if path_str.starts_with(&format!("{}.", prefix)) {
                        ...
                        if first_seg_is_known || is_passed_alias {
                ...
                if let Some(var_id) = self.reactive_vars.get(&effective_path) {
                    return Ok((var_id.clone(), None));
                }
                if let Some(var_expr) = self.compiler.get_var_expr(&effective_path) {
                    if matches!(var_expr.node, Expression::Link) {
                        let var_id = VarId::new(effective_path.as_str());
                        ...
                        self.reactive_vars
                            .insert(effective_path.clone(), var_id.clone());
                        return Ok((var_id, Some(effective_path)));
                    }
                    if self.compiler.is_reactive(var_expr) {
                        return Ok((VarId::new(effective_path.as_str()), None));
                    }
                }
                if is_passed_alias {
                    let passed_stripped = format!("{}.{}", PASSED_VAR, path_str);
                    if let Some(var_id) = self.reactive_vars.get(passed_stripped.as_str()) {
                        return Ok((var_id.clone(), None));
                    }
                    if let Some(var_expr) = self.compiler.get_var_expr(passed_stripped.as_str()) {
                        if matches!(var_expr.node, Expression::Link) {
                            ...
                            self.reactive_vars
                                .insert(passed_stripped.clone(), var_id.clone());
                            return Ok((var_id, Some(passed_stripped)));
                        }
                        if self.compiler.is_reactive(var_expr) {
                            return Ok((VarId::new(passed_stripped.as_str()), None));
                        }
                    }
                }
                ...
                if effective_path.contains(".event.") {
                    ...
                    if let Some(field_suffix) = effective_path
                        .strip_prefix(&format!("{link_path}."))
                        .filter(|suffix| !suffix.is_empty())
                    {
                        let projected_var = self.fresh_var("link_input_field");
                        let field_path = field_suffix.to_string();
                        self.collections.insert(
                            projected_var.clone(),
                            CollectionSpec::Map {
                                source: input_var,
                                f: Arc::new(move |value: &Value| {
                                    ...
                                    for part in field_path.split('.') {
                                        let Some(next) = current.get_field(part).cloned() else {
                                            return Value::Unit;
                                        };
                                        current = next;
                                    }
                        ...
                        return Ok((projected_var, Some(link_path)));
                    }
                ...
                // Follow simple alias chains for event sources like:
                //   edit_committed: event_ports.edit_committed
                // so HOLD/LATEST inputs can resolve through the alias to the underlying LINK path.
                for candidate_name in [effective_path.as_str(), var_name.as_str()] {
                    if let Some(var_expr) = self.compiler.get_var_expr(candidate_name) {
                        if let Some(target_path) = alias_target_path(var_expr) {
                            if target_path != path_str && target_path != effective_path {
                                if let Ok(resolved) = self.compile_event_source(var_expr) {
                                    return Ok(resolved);
                                }
                            }
                        }
    ...
    fn compile_document_expr(&mut self, expr: &Spanned<Expression>) -> Result<VarId, String> {
        ...
        // Track the actual reactive dependencies used by the document tree,
        // including synthesized `.text` state vars for text inputs. Filter to
        // deps with guaranteed initial values so document evaluation can render
        // immediately without blocking on event-only streams.
        let mut reactive_deps = self.find_reactive_deps_in_expr(expr);
        self.collect_ensured_text_reactive_deps(expr, &mut reactive_deps);
        let mut seen_reactive_deps = HashSet::new();
        let reactive_deps: Vec<String> = reactive_deps
            .into_iter()
            .filter(|name| seen_reactive_deps.insert(name.clone()))
            .filter(|name| {
                self.reactive_vars
                    .get(name)
                    .is_some_and(|var_id| self.has_initial_value(var_id))
            })
            .collect();
        #[cfg(test)]
        if trace_cells {
        ...
        if reactive_deps.len() == 1 {
            ...
            if self.expr_contains_user_function_call(expr) {
                ...
                if trace_cells {
                    eprintln!(
                        "[cells-dd] using multi-dep document closure for {dep_name} due to user function calls"
                    );
                }
        ...
        if trace_compile {
            eprintln!("[dd-trace] before find_root_and_derived");
        }
        match self.find_root_and_derived(&reactive_deps) {
            Ok((root_dep, derived_deps)) => {
                if trace_compile {
                    eprintln!(
                        "[dd-trace] find_root_and_derived ok root={} derived={}",
                        root_dep,
                        derived_deps.join(", ")
                    );
                }
            ...
            Err(_) => {
                ...
                if trace_compile {
                    eprintln!("[dd-trace] before compile_multi_dep_document (independent deps)");
                }
                return self.compile_multi_dep_document(expr, &reactive_deps);
            }
        }
    ...
    fn compile_multi_dep_document(
        ...
        if trace_compile {
            eprintln!(
                "[dd-trace] compile_multi_dep_document real_dep_names={}",
                real_dep_names.join(", ")
            );
        }
        ...
        if real_dep_names.len() == 1 {
            ...
            let dep_var = self.reactive_vars.get(dep_name).unwrap().clone();
            if trace_compile {
                eprintln!("[dd-trace] before build_multi_dep_doc_closure (single dep)");
            }
        ...
        if trace_compile {
            eprintln!("[dd-trace] before build_multi_dep_doc_closure (multi dep)");
        }
        let doc_closure = self.build_multi_dep_doc_closure(doc_expr, &real_dep_names)?;
        let doc_var = self.fresh_var("document");
        self.collections.insert(
    ...
    fn build_multi_dep_doc_closure(
        ...
        if trace_compile {
            eprintln!(
                "[dd-trace] entered build_multi_dep_doc_closure deps={}",
                dep_names.join(", ")
            );
        }
        let compiler = self.compiler.clone();
        if trace_compile {
            eprintln!("[dd-trace] cloned compiler");
        }
        let doc_expr = doc_expr.clone();
        if trace_compile {
            eprintln!("[dd-trace] cloned doc_expr");
        }
        let dep_names: Vec<String> = dep_names.to_vec();
        if trace_compile {
            eprintln!("[dd-trace] copied dep_names");
        }
        let keyed_list_short_name = self.keyed_display_list_name.clone();
        if trace_compile {
            eprintln!("[dd-trace] cloned keyed_list_short_name");
        }
        ...
        if trace_compile {
            eprintln!("[dd-trace] collected state_var_names");
        }
        ...
        for (var_name, var_expr) in self.compiler.variables.iter() {
            if let Some(field_name) = var_name.strip_prefix("store.") {
                // Skip reactive store vars entirely.
                // Live document closures reconstruct those from the combined dep payload
                // and link-path injection. Trying to statically evaluate reactive store
                // fields here can recurse back into the document closure path.
                if self.reactive_vars.contains_key(var_name)
                    || dep_names.iter().any(|d| d == var_name)
                {
                    continue;
                }
                if trace_compile {
                    eprintln!("[dd-trace] link-default candidate {var_name}");
                }
            ...
            if trace_compile {
                eprintln!("[dd-trace] about to eval static scope var {var_name}");
            }
            if let Ok(val) = self
                .compiler
                .eval_static_with_scope(var_expr, &IndexMap::new())
            {
                if trace_compile {
                    eprintln!("[dd-trace] static-scope candidate {var_name}");
                }
        ...
        Ok(Arc::new(move |combined: &Value| {
            ...
            let mut object_values: std::collections::HashMap<String, Value> =
            ...
            for (i, name) in dep_names.iter().enumerate() {
                ...
                if parts.len() >= 2 {
                    ...
                    let existing = object_values
                        .get(&root_name)
                        .cloned()
                        .unwrap_or_else(|| Value::Object(Arc::new(BTreeMap::new())));
                    let updated = set_nested_field(&existing, &nested_path, decorated.clone());
                    object_values.insert(root_name, updated);
                }
                scope.insert(name.clone(), decorated);
            }

            // Build composite objects for each parent prefix
            for (parent_name, value) in &object_values {
                if parent_name == "store" {
                    // store gets special handling below (LINK stubs, link path injection)
                    continue;
                }
                scope.insert(parent_name.clone(), value.clone());
            }

            let mut store_value = object_values
                .remove("store")
                .unwrap_or_else(|| Value::Object(Arc::new(BTreeMap::new())));

            let has_nested_field = |value: &Value, path: &str| {
                let mut current = value;
                for part in path.split('.') {
                    let Some(next) = current.get_field(part) else {
                        return false;
                    };
                    current = next;
                }
            ...
            for (field_name, default_value) in &link_defaults {
                if !has_nested_field(&store_value, field_name) {
                    store_value = set_nested_field(&store_value, field_name, default_value.clone());
                }
            }

            // Inject __link_path__ for LINK variables (top-level buttons, filter links, etc.)
            for (nested_path, full_path) in &link_injections {
                let merged = get_nested_field(&store_value, nested_path)
                    .map(|existing| {
                        if matches!(existing, Value::Object(_)) {
                            existing
                                .clone()
                                .update_field(LINK_PATH_FIELD, Value::text(full_path.as_str()))
                        } else {
                ...
                store_value = set_nested_field(&store_value, nested_path, merged);
            }
            ...
            if !matches!(store_value, Value::Unit) {
        ...
        match transform {
            ...
            HoldTransform::Custom => {
                ...
                if let Some(then_expr) = then_body {
                    ...
                    let depends_on_state = Self::expr_references_name(&then_expr, state_name);
                    Arc::new(move |state: &Value, event: &Value| {
                        if matches!(event, Value::Unit) || event.get_tag() == Some("SKIP") {
                            return state.clone();
                        }
                        if !depends_on_state && event.get_tag() != Some("Event") {
                            return event.clone();
                        }
                        let mut scope = IndexMap::new();
                        ...
                        if let Some(event_name) = &event_name {
                            let event_value = event
                                .get_field("__hold_event")
                                .cloned()
                                .unwrap_or_else(|| event.clone());
                            if matches!(event_value, Value::Unit)
                                || event_value.get_tag() == Some("SKIP")
                            {
                                return state.clone();
                            }
                            inject_scope_path_merged(&mut scope, event_name, event_value);
                        }
                        let result = compiler
                    ...
                    // State-dependent THEN bodies are wrapped in per-arm markers
                    // during compile_hold_body_events so we can evaluate only the
                    // matching THEN body here.
                    let arm_transforms: Vec<(String, Option<String>, Spanned<Expression>)> = inputs
                        .iter()
                        .enumerate()
                        .filter_map(|(index, input)| {
                            if let Expression::Pipe { from, to } = &input.node {
                                if let Expression::Then { body } = &to.node {
                                    return Some((
                                        format!("{hold_name}__latest_{index}"),
                                        Self::extract_hold_event_source_name(from),
                                        body.as_ref().clone(),
                                    ));
                                }
                    ...
                    Arc::new(move |state: &Value, event: &Value| {
                            ...
                            event.get_field("__hold_then").and_then(|value| value.as_text())
                        {
                            ...
                            scope.insert(sname.clone(), state.clone());
                            if let Some(event_name) = event_name {
                                if let Some(event_value) = event.get_field("__hold_event") {
                        ...
                        // Non-marker event values are already the final state payload.
                    ...
                    // No THEN body found — event replaces state, except Unit/SKIP
                    // which mean "no update" for bare alias-driven HOLD bodies.
                    Arc::new(|state: &Value, event: &Value| {
                        if matches!(event, Value::Unit) || event.get_tag() == Some("SKIP") {
                            state.clone()
                        } else {
                            event.clone()
    ...
    fn collect_ensured_text_reactive_deps_with_visited(
        ...
        match &expr.node {
            Expression::Pipe { from, to } => {
                self.collect_ensured_text_reactive_deps_with_visited(from, deps, visited_functions);
                self.collect_ensured_text_reactive_deps_with_visited(to, deps, visited_functions);
            }
            Expression::FunctionCall { arguments, .. } => {
                ...
                let fn_name = match &expr.node {
                    Expression::FunctionCall { path, .. } => Some(
                        path.iter()
                ...
                if let Some(fn_name) = fn_name
                    && let Some((qualified_name, _params, body)) =
                        self.compiler.find_function(&fn_name)
                    && visited_functions.insert(qualified_name.clone())
                {
                    self.collect_ensured_text_reactive_deps_with_visited(
                        body,
                        deps,
    ...
    fn build_when_map_fn(
        ...
        // Keep track of whether a non-literal arm is a real binding pattern,
        // because wildcard `__ => SKIP` must suppress the event instead of
        // falling through as a pass-through binding.
        ...
        for arm in arms {
            ...
            let is_binding_pattern = matches!(
                ...
                static_expression::Pattern::Alias { name }
                    if {
                        let s = name.as_str();
                        s != "__" && s != "_" && !s.chars().next().map_or(false, |c| c.is_uppercase())
                    }
            );
            compiled_arms.push((pattern_val, body_val, is_binding_pattern));
        }

        Arc::new(move |input: &Value| {
            for (pattern_val, body_val, is_binding_pattern) in &compiled_arms {
                match pattern_val {
                    Some(pv) => {
                        if input == pv {
                    ...
                    None => {
                        // Wildcards and bindings both match anything, but only
                        // real bindings are allowed to fall through as pass-through
                        // when the body cannot be statically evaluated.
                        if *is_binding_pattern && *body_val == Value::Unit {
                            return input.clone();
                        }
                        return body_val.clone();
                    }
    ...
    fn when_has_skip_and_text_dep(
        ...
        // This optimization is only valid for key_down-driven WHENs whose body
        // depends on the live text input state (for example `input.text` or
        // `input.event.change.text`). Plain scalar/keyed Enter/Escape handlers
        // must use the normal WHEN compilation path.
        let Expression::Alias(Alias::WithoutPassed { parts, .. }) = &from.node else {
            return false;
        };
        ...
        let Some(input_base) = effective.split(".event.key_down").next() else {
            return false;
        };
        if input_base == effective {
            return false;
        }

        let text_prefix = format!("{input_base}.text");
        let change_prefix = format!("{input_base}.event.change");
        let keydown_text_prefix = format!("{input_base}.event.key_down.text");
        arms.iter()
            .filter(|arm| !matches!(arm.body.node, Expression::Skip))
            .any(|arm| {
                self.expr_references_any_path_prefix(
                    &arm.body,
                    &[&text_prefix, &change_prefix, &keydown_text_prefix],
                )
            })
    }

    fn expr_references_any_path_prefix(
        &self,
        expr: &Spanned<Expression>,
        prefixes: &[&str],
    ) -> bool {
        match &expr.node {
            Expression::Alias(Alias::WithoutPassed { parts, .. }) => {
                ...
                prefixes.iter().any(|prefix| {
                    raw_path == *prefix
                        || raw_path.starts_with(&format!("{prefix}."))
                        || effective_path == *prefix
                        || effective_path.starts_with(&format!("{prefix}."))
                })
            }
            Expression::Alias(Alias::WithPassed { extra_parts }) => {
                let raw_path = extra_parts
                    .iter()
                    .map(|p| p.as_str())
                    .collect::<Vec<_>>()
                    .join(".");
            ...
            Expression::Pipe { from, to } => {
                self.expr_references_any_path_prefix(from, prefixes)
                    || self.expr_references_any_path_prefix(to, prefixes)
            }
            Expression::FunctionCall { arguments, .. } => arguments.iter().any(|argument| {
                argument
                    ...
                    .value
                    .as_ref()
                    .map(|value| self.expr_references_any_path_prefix(value, prefixes))
                    .unwrap_or(false)
            }),
            Expression::List { items } => items
                .iter()
                .any(|item| self.expr_references_any_path_prefix(item, prefixes)),
            Expression::Object(obj) => obj
                .variables
                .iter()
                .any(|var| self.expr_references_any_path_prefix(&var.node.value, prefixes)),
            Expression::Block { variables, output } => {
                variables
                    .iter()
                    .any(|var| self.expr_references_any_path_prefix(&var.node.value, prefixes))
                    || self.expr_references_any_path_prefix(output, prefixes)
            }
            _ => false,
        }
    }
                ...
                transform: Arc::new(|state: &Value, event: &Value| {
                    ...
                    match event_type.as_str() {
                        ...
                        "key" => {
                            let key = event
                                .get_field("key")
                                .and_then(|v| v.as_text().or_else(|| v.as_tag()))
                                .unwrap_or("")
                                .to_string();
                            if key == "Enter" {
                                let display = state
                                ...
                                let live_text = event
                                    ...
                                    .to_string();
                                let trimmed = live_text.trim().to_string();
                                if !trimmed.is_empty() {
                                    Value::object([
                                        ("display", Value::text("")),
    ...
    fn try_compile_inline_static_event_map(
        ...
        let source_name = Self::resolve_static_source_base_path(source_expr, &IndexMap::new());
        let compiler = self.compiler.clone();
        let mut event_vars = Vec::new();
        ...
        for (idx, item) in source_value.list_items().into_iter().enumerate() {
            ...
            let mut item_scope = IndexMap::new();
            item_scope.insert(item_param.clone(), decorated_item.clone());

            if let Expression::Pipe { from, to } = &new_expr.node
            ...
            for (kind, link_path, event_scope_path) in bindings {
                ...
                let event_var = self.fresh_var("static_item_event_map");
                let new_expr_clone = new_expr.clone();
                let compiler_clone = compiler.clone();
                let item_scope_clone = item_scope.clone();
                self.collections.insert(
                    event_var.clone(),
                    CollectionSpec::Map {
                        source: input_var,
                        f: Arc::new(move |event: &Value| {
                            let mut scope = item_scope_clone.clone();
                            scope.insert("__event".to_string(), event.clone());
                            scope.insert("event_value".to_string(), event.clone());
                            inject_scope_path_merged(&mut scope, &event_scope_path, event.clone());
                            compiler_clone.eval_static_tolerant(&new_expr_clone, &scope)
                        }),
                    },
                );
                event_vars.push(event_var);
    ...
    fn collect_static_event_bindings(
        &self,
        compiler: &Compiler,
        expr: &Spanned<Expression>,
        scope: &IndexMap<String, Value>,
    ) -> Vec<(InputKind, String, String)> {
        let mut bindings = Vec::new();
        self.collect_static_event_bindings_into(compiler, expr, scope, &mut bindings);
        bindings
    }

    fn collect_static_event_bindings_into(
        &self,
        compiler: &Compiler,
        expr: &Spanned<Expression>,
        scope: &IndexMap<String, Value>,
        out: &mut Vec<(InputKind, String, String)>,
    ) {
        if let Some(binding) = Self::resolve_static_event_binding(expr, scope) {
        ...
        match &expr.node {
            Expression::Pipe { from, to } => {
                if let Expression::FunctionCall { path, arguments } = &to.node {
                    ...
                    if path_strs.as_slice() == ["List", "map"] {
                        ...
                        let item_param = arguments
                            ...
                            .map(|arg| arg.node.name.as_str().to_string())
                            .unwrap_or_else(|| "item".to_string());
                        let Some(new_expr) = arguments
                            .iter()
                        ...
                        let source_name = Self::resolve_static_source_base_path(from, scope);
                        for (idx, item) in source_value.list_items().into_iter().enumerate() {
                            let key = format!("{idx:04}");
                            let decorated_item = if let Some(source_name) = source_name.as_deref() {
                                decorate_reactive_scope_value(&item, &format!("{source_name}.{key}"))
                            ...
                            nested_scope.insert(item_param.clone(), decorated_item);
                            self.collect_static_event_bindings_into(
                                compiler,
                                new_expr,
                                &nested_scope,
                                out,
                ...
                let bindings_before = out.len();
                self.collect_static_event_bindings_into(compiler, from, scope, out);
                if out.len() == bindings_before {
                    self.collect_static_event_bindings_into(compiler, to, scope, out);
                }
            }
            Expression::FunctionCall { path, arguments } => {
                let fn_name = path
                    ...
                    .collect::<Vec<_>>()
                    .join("/");
                let path_parts: Vec<&str> = path.iter().map(|segment| segment.as_str()).collect();
                if let Some((qualified_name, params, body)) = compiler.find_function(&fn_name) {
                    let scoped = compiler.with_module_context(qualified_name);
            ...
            Expression::Block { variables, output } => {
                ...
                for variable in variables {
                    if compiler.expr_contains_link(&variable.node.value) {
                        self.collect_static_event_bindings_into(
                            compiler,
                            &variable.node.value,
                            &block_scope,
                    ...
                    let mut value =
                        compiler.eval_static_tolerant(&variable.node.value, &block_scope);
                    if compiler.expr_contains_link(&variable.node.value)
                        && value_needs_reactive_decoration(&value)
                        && let Some(base_path) = Compiler::scope_root_path(scope)
                    {
                        value = decorate_reactive_scope_value(
                            &value,
                            &format!("{base_path}.{}", variable.node.name.as_str()),
                        );
                    }
                    block_scope.insert(variable.node.name.as_str().to_string(), value);
                }
                self.collect_static_event_bindings_into(compiler, output, &block_scope, out);
            }
            Expression::Latest { inputs } => {
                for input in inputs {
                    self.collect_static_event_bindings_into(compiler, input, scope, out);
                }
            }
            Expression::Then { body } => {
                self.collect_static_event_bindings_into(compiler, body, scope, out);
            }
            Expression::When { arms } | Expression::While { arms } => {
                for arm in arms {
                    self.collect_static_event_bindings_into(compiler, &arm.body, scope, out);
                }
            }
            _ => {}
        }
    ...
    fn resolve_static_event_binding(
        expr: &Spanned<Expression>,
        scope: &IndexMap<String, Value>,
    ) -> Option<(InputKind, String, String)> {
        let Expression::Alias(Alias::WithoutPassed { parts, .. }) = &expr.node else {
            return None;
        };
        let root_name = parts.first()?.as_str().to_string();
        let root_value = scope.get(&root_name)?;
        let suffix = parts[1..]
            .iter()
            .map(|p| p.as_str())
            .collect::<Vec<_>>()
        ...
        let (kind, link_path) = if let Some(resolved) =
            Self::resolve_static_item_event_path(expr, &root_name, root_value)
        {
            resolved
        } else {
            let root_base = root_value
                .get_field(LINK_PATH_FIELD)
                .and_then(|v| v.as_text())
                .map(|s| s.to_string())
                .or_else(|| {
                    root_value
                        .get_field(HOVER_PATH_FIELD)
                        .and_then(|v| v.as_text())
                        .map(|s| s.strip_suffix(".hovered").unwrap_or(s).to_string())
                })?;
            Self::detect_event_kind_and_path(&format!("{root_base}.{suffix}"))
        };
        let scope_path = Self::normalize_event_binding_path(&format!("{root_name}.{suffix}"));
        Some((kind, link_path, scope_path))
    }

    fn compile_static_link_forwarders(
        &mut self,
        doc_expr: &Spanned<Expression>,
    ) -> Result<(), String> {
        let compiler = self.compiler.clone();
        ...
        if trace_cells {
            std::println!(
                "[cells-dd] compile_static_link_forwarders count={}",
                forwarders.len()
            );
            for forwarder in &forwarders {
                std::println!(
                    "[cells-dd] forwarder target={} kind={:?} link={} scope_path={}",
                    forwarder.target_path,
                    forwarder.kind,
                    forwarder.link_path,
                    forwarder.event_scope_path
                );
            }
        }
        ...
        let mut input_vars_by_source: std::collections::BTreeMap<String, VarId> =
            std::collections::BTreeMap::new();
        let mut grouped_event_vars: std::collections::BTreeMap<String, Vec<VarId>> =
            std::collections::BTreeMap::new();

        for forwarder in forwarders {
            let input_key = format!("{:?}|{}", forwarder.kind, forwarder.link_path);
            let input_var = if let Some(existing) = input_vars_by_source.get(&input_key) {
                existing.clone()
            } else {
                let input_id =
                    self.add_input(forwarder.kind.clone(), Some(forwarder.link_path.clone()));
                let input_var = self.fresh_var("static_link_forward_input");
                self.collections
                    .insert(input_var.clone(), CollectionSpec::Input(input_id));
                input_vars_by_source.insert(input_key, input_var.clone());
                input_var
            };

            let event_var = self.fresh_var("static_link_forward_map");
            let compiler = compiler.clone();
            let source_expr = forwarder.source_expr.clone();
            let event_scope_path = forwarder.event_scope_path.clone();
            let base_scope = forwarder.scope.clone();
            let reactive_dep_names = self.filter_static_link_forwarder_deps(
                self.find_sampled_reactive_deps_in_expr(&source_expr),
            );
            ...
            for dep_name in &reactive_dep_names {
                ...
                let dep_index = sampled_dep_names.len() + 1;
                let sample_var = self.fresh_var("static_link_forward_sample");
                if sampled_dep_names.is_empty() {
                    self.collections.insert(
                        sample_var.clone(),
                        CollectionSpec::SampleOnEvent {
                            event: sampled_source,
                            dep: dep_var,
                            f: Arc::new(move |event: &Value, dep: &Value| {
                                Value::object([
                                    (format!("{DEP_FIELD_PREFIX}0"), event.clone()),
                                    (format!("{DEP_FIELD_PREFIX}{dep_index}"), dep.clone()),
                                ])
                            }),
                        },
                    );
                    ...
                    self.collections.insert(
                        ...
                        CollectionSpec::SampleOnEvent {
                            ...
                            dep: dep_var,
                            f: Arc::new(move |combined: &Value, dep: &Value| {
                                combined.update_field(
                                    &format!("{DEP_FIELD_PREFIX}{dep_index}"),
                                    dep.clone(),
                                )
                            }),
                        },
                ...
                sampled_source = sample_var;
                sampled_dep_names.push(dep_name.clone());
            }
            self.collections.insert(
                event_var.clone(),
                CollectionSpec::Map {
                    ...
                    f: Arc::new(move |combined: &Value| {
                        let event = if sampled_dep_names.is_empty() {
                            combined.clone()
                        } else {
                            combined
                        ...
                        inject_scope_path_merged(&mut scope, &event_scope_path, event.clone());
                        if !sampled_dep_names.is_empty() {
                            for (idx, dep_name) in sampled_dep_names.iter().enumerate() {
                                let dep_key = format!("{DEP_FIELD_PREFIX}{}", idx + 1);
                                let dep_value =
                                    combined.get_field(&dep_key).cloned().unwrap_or(Value::Unit);
                                let decorated = decorate_reactive_scope_value(&dep_value, dep_name);
                                ...
                                if dep_name.contains('.') {
                                    ...
                                    scope.insert(parts[0].to_string(), updated);
                                }
                            }
                        }
                        compiler.eval_static_tolerant(&source_expr, &scope)
                    }),
                },
            );
            grouped_event_vars
                .entry(forwarder.target_path)
                .or_default()
                .push(event_var);
        }

        for (target_path, mut event_vars) in grouped_event_vars {
            let public_var = VarId::new(target_path.as_str());
            let source_var = if event_vars.len() == 1 {
                event_vars.pop().expect("single event forwarder")
            } else {
                let concat_var = self.fresh_var("static_link_forward_concat");
                self.collections
                    .insert(concat_var.clone(), CollectionSpec::Concat(event_vars));
                concat_var
            };
            self.collections.insert(
                ...
                CollectionSpec::Map {
                    source: source_var,
                    f: Arc::new(|value: &Value| value.clone()),
                },
            );
            self.reactive_vars.insert(target_path, public_var);
        }

        Ok(())
    }

    fn filter_static_link_forwarder_deps(&self, deps: Vec<String>) -> Vec<String> {
        ...
        deps.into_iter()
            .filter(|name| {
                ...
                if let Some(var_id) = self.reactive_vars.get(name)
                    && let Some(spec) = self.collections.get(var_id)
                    && matches!(spec, CollectionSpec::SideEffect { .. })
                {
                    return false;
                }
                if let Some(expr) = self.compiler.get_var_expr(name)
                    && matches!(expr.node, Expression::Link)
                {
                    return false;
                }
                true
    ...
    fn collect_static_link_forwarders_into(
        &self,
        compiler: &Compiler,
        expr: &Spanned<Expression>,
        scope: &IndexMap<String, Value>,
        out: &mut Vec<StaticLinkForwarder>,
        ...
        match &expr.node {
            Expression::Alias(Alias::WithoutPassed { parts, .. }) if parts.len() == 1 => {
                let alias_name = parts[0].as_str().to_string();
                if !active_aliases.insert(alias_name.clone()) {
                    return;
                }
                if let Some(var_expr) = compiler.get_var_expr(alias_name.as_str()) {
                    self.collect_static_link_forwarders_into(
                        compiler,
                        var_expr,
                        scope,
                ...
                active_aliases.remove(alias_name.as_str());
            }
            Expression::Pipe { from, to } => {
                if let Expression::FunctionCall { path, arguments } = &to.node {
                    let path_strs: Vec<&str> = path.iter().map(|segment| segment.as_str()).collect();
                ...
                if let Expression::LinkSetter { alias } = &to.node
                    && let Some(target_path) = Self::resolve_static_link_target(&alias.node, scope)
                {
                    let bindings = self.collect_static_event_bindings(compiler, from, scope);
                    for (kind, link_path, event_scope_path) in bindings {
                        out.push(StaticLinkForwarder {
                            target_path: target_path.clone(),
                            kind,
                            link_path,
                            event_scope_path,
                            source_expr: from.as_ref().clone(),
                            scope: scope.clone(),
                        });
                    }
                }
            ...
            Expression::FunctionCall { path, arguments } => {
                ...
                let path_parts: Vec<&str> = path.iter().map(|segment| segment.as_str()).collect();
                if let Some((qualified_name, params, body)) = compiler.find_function(&fn_name) {
                    let active_function = qualified_name.clone();
                    if !active_functions.insert(active_function.clone()) {
                        return;
                    }
                    let scoped = compiler.with_module_context(qualified_name);
                    let mut fn_scope = scope.clone();
    ...
    fn resolve_static_link_target(
        alias: &Alias,
        scope: &IndexMap<String, Value>,
    ) -> Option<String> {
        match alias {
            Alias::WithoutPassed { parts, .. } => Some(
                parts
                    .iter()
                    .map(|part| part.as_str())
                    .collect::<Vec<_>>()
            ...
            Alias::WithPassed { extra_parts } => {
                ...
                current
                    ...
                    .map(|path| path.to_string())
                    .or_else(|| current.as_text().map(|text| text.to_string()))
                    .or_else(|| {
                        Some(
                            extra_parts
                                .iter()
    ...
    fn should_traverse_static_structure_arg(path_parts: &[&str], arg_name: &str) -> bool {
        matches!(
            (path_parts, arg_name),
            (["Document", "new"], "root")
                | (["Scene", "new"], "root")
                | (["Element", "stripe"], "items")
                | (["Scene", "Element", "stripe"], "items")
                | (["Element", "stack"], "layers")
                | (["Scene", "Element", "stack"], "layers")
                | (["Element", "container"], "content")
                | (["Scene", "Element", "container"], "content")
                | (["Element", "paragraph"], "content")
                | (["Scene", "Element", "paragraph"], "content")
        )
    }
    ...
    fn compile_inline_keyed_retain(
        ...
        let mut reactive_deps = self.find_reactive_deps_preferring_text_state(predicate_expr);
        ...
        if reactive_deps.is_empty() {
            ...
            // Preserve a real initial value when the reactive dep already has one.
            // Only synthesize a fallback wrapper for streams that truly start empty.
            let filter_state = if self.has_initial_value(&reactive_var) {
                reactive_var
            } else {
    ...
    fn ensure_text_state_var(&mut self, path: &str) -> Option<String> {
        ...
        self.collections
            .insert(input_var.clone(), CollectionSpec::Input(input_id));
        let input_text_var = self.fresh_var("text_state_value");
        self.collections.insert(
            input_text_var.clone(),
            CollectionSpec::Map {
                source: input_var,
                f: Arc::new(|value: &Value| {
                    value
                        .get_field("text")
                        .cloned()
        ...
        self.collections.insert(
            state_var.clone(),
            CollectionSpec::HoldLatest(vec![default_var, input_text_var]),
        );
        self.reactive_vars
    ...
    {
        match &expr.node {
            Expression::Variable(var) => self.collect_list_chain_ops(&var.value, ops),
            Expression::Alias(Alias::WithoutPassed { parts, .. }) => {
                let path = parts
                    .iter()
                    .map(|part| part.as_str())
                ...
                if let Some(prefix) = &self.scope_prefix {
                    candidate_names.push(format!("{prefix}.{path}"));
                }
                candidate_names.push(path);

                for candidate in candidate_names {
                    if let Some(target_expr) = self.compiler.get_var_expr(&candidate) {
                        if target_expr.span.start == expr.span.start
                            && target_expr.span.end == expr.span.end
                        {
                            continue;
                        }
                        return self.collect_list_chain_ops(target_expr, ops);
                    }
                }
                Ok(expr)
            }
        ...
        let classify: ClassifyFn = Arc::new(|v: &Value| {
            ...
            let classified = if subpath.contains("remove_todo_button") && event_tag == "Press" {
                ...
                if subpath.contains(".event.key_down") {
                    let key = match event_value
                        .get_field("key")
                        .and_then(|value| value.as_text().or_else(|| value.as_tag()))
                    {
                        Some(key) => key,
                        None => return None,
                    };
                        ...
                        if let Expression::Pipe { from, to } = &on_expr.node {
                            if let Expression::Then { body } = &to.node {
                                if !Self::expr_references_name(from, "item") {
                                    ...
                                    let reactive_deps =
                                        self.find_sampled_reactive_deps_in_expr(body);
                                    if reactive_deps.is_empty() {
                                        let remove_then_var =
    ...
    fn resolve_static_source_base_path(
        expr: &Spanned<Expression>,
        scope: &IndexMap<String, Value>,
    ) -> Option<String> {
        let Expression::Alias(Alias::WithoutPassed { parts, .. }) = &expr.node else {
            return Self::resolve_static_source_name(expr);
        };
        let root_name = parts.first()?.as_str();
        let mut current = scope.get(root_name).cloned();
        if let Some(mut value) = current.take() {
            for part in &parts[1..] {
                value = value.get_field(part.as_str()).cloned()?;
            }
            if let Some(path) = Self::reactive_value_base_path(&value) {
                return Some(path);
            }
        }
        Self::resolve_static_source_name(expr)
    }

    fn reactive_value_base_path(value: &Value) -> Option<String> {
        match value {
            ...
            Value::Tagged { tag, fields } if tag.as_ref() == LIST_TAG => {
                let (first_key, first_item) = fields.iter().next()?;
                let item_path = Self::reactive_value_base_path(first_item)?;
                item_path
                    .strip_suffix(&format!(".{}", first_key.as_ref()))
                    .map(|path| path.to_string())
                    .or_else(|| item_path.rsplit_once('.').map(|(path, _)| path.to_string()))
    ...
    fn find_reactive_deps_in_expr(&self, expr: &Spanned<Expression>) -> Vec<String> {
        ...
        let local_bindings = IndexMap::new();
        let passed_bindings = IndexMap::new();
        self.collect_reactive_deps(
            expr,
            &mut deps,
            &mut visited_functions,
            &local_bindings,
    ...
    fn find_reactive_deps_preferring_text_state(
        &mut self,
        expr: &Spanned<Expression>,
    ) -> Vec<String> {
        let mut deps = self.find_reactive_deps_in_expr(expr);
        let mut text_deps = Vec::new();
        self.collect_ensured_text_reactive_deps(expr, &mut text_deps);
        for text_dep in text_deps {
            if let Some(link_dep) = text_dep.strip_suffix(".text") {
                deps.retain(|dep| dep != link_dep);
            }
            if !deps.contains(&text_dep) {
                deps.push(text_dep);
            }
        }
        deps
    ...
    fn find_sampled_reactive_deps_in_expr(&self, expr: &Spanned<Expression>) -> Vec<String> {
        let mut deps = Vec::new();
        let mut visited_functions = std::collections::HashSet::new();
        self.collect_sampled_reactive_deps(expr, &mut deps, &mut visited_functions);
        deps
    }
    ...
    fn collect_reactive_deps(
        ...
        match &expr.node {
            ...
            Expression::Alias(alias) => {
                ...
                let mut candidates = self.reactive_candidate_prefixes(&full_path);
                candidates.extend(self.reactive_candidate_prefixes(&path));
                if let Some(resolved) =
                    self.resolve_alias_binding_path(alias, local_bindings, passed_bindings)
                {
                    candidates.extend(self.reactive_candidate_prefixes(&resolved));
                }

                for candidate in candidates {
                    if self.is_known_reactive_name(&candidate) {
                        deps.push(candidate);
                        break;
                    }
                }
            ...
            Expression::Pipe { from, to } => {
                ...
                if self.collect_piped_call_reactive_deps(
                    from,
                    to,
                    deps,
                    visited_functions,
            ...
            Expression::TextLiteral { parts, .. } => {
                for part in parts {
                    if let TextPart::Interpolation { var, .. } = part {
                        ...
                        candidates.extend(self.reactive_candidate_prefixes(&path));
                        if let Some(resolved) =
                            self.resolve_path_binding_path(&path, local_bindings, passed_bindings)
                        {
                            candidates.extend(self.reactive_candidate_prefixes(&resolved));
                        }
                        for candidate in candidates {
            ...
            Expression::Block { variables, output } => {
                let mut scoped_locals = local_bindings.clone();
                for variable in variables {
                    self.collect_reactive_deps(
                        &variable.node.value,
                        deps,
                    ...
                    if let Some(resolved) = self.resolve_expr_binding_path(
                        &variable.node.value,
                        &scoped_locals,
                        passed_bindings,
                    ) {
                        scoped_locals.insert(variable.node.name.to_string(), resolved);
                    }
                }
                self.collect_reactive_deps(
                    output,
    ...
    fn build_passed_binding_paths(
        ...
        let mut bindings = IndexMap::new();
        let Some(pass_expr) = arguments
            .iter()
            .find(|arg| arg.node.name.as_str() == "PASS")
            .and_then(|arg| arg.node.value.as_ref())
        else {
            return bindings;
        };
        let Expression::Object(object) = &pass_expr.node else {
            return bindings;
        };
        for var in &object.variables {
            if let Some(resolved) =
                self.resolve_expr_binding_path(&var.node.value, local_bindings, passed_bindings)
            {
                bindings.insert(var.node.name.to_string(), resolved);
            }
        }
        bindings
    }

    fn build_function_local_binding_paths(
        &self,
        params: &[String],
        arguments: &[Spanned<Argument>],
        local_bindings: &IndexMap<String, String>,
        ...
        for arg in arguments {
            ...
            let Some(value) = arg.node.value.as_ref() else {
                continue;
            };
            if let Some(resolved) =
                self.resolve_expr_binding_path(value, local_bindings, passed_bindings)
            {
                bindings.insert(arg.node.name.to_string(), resolved);
            }
        }
        ...
        for param_name in params {
            ...
            let Some(arg) = arguments
                .iter()
                .find(|arg| arg.node.name.as_str() == param_name.as_str())
            else {
                continue;
            };
            let Some(value) = arg.node.value.as_ref() else {
            ...
            if let Some(resolved) =
                self.resolve_expr_binding_path(value, local_bindings, passed_bindings)
            {
                bindings.insert(param_name.clone(), resolved);
            }
        }
    ...
    fn resolve_expr_binding_path(
        ...
        match &expr.node {
            Expression::Variable(var) => {
                self.resolve_expr_binding_path(&var.value, local_bindings, passed_bindings)
            }
            Expression::Alias(alias) => {
                self.resolve_alias_binding_path(alias, local_bindings, passed_bindings)
            }
    ...
    fn resolve_path_binding_path(
        ...
        let direct = if from_passed {
            format!("PASSED.{}", parts.join("."))
        } else {
            parts.join(".")
        };
        ...
        if let Some(bound_prefix) = binding_map.get(parts[0]) {
            let suffix = if parts.len() > 1 {
                format!(".{}", parts[1..].join("."))
            } else {
                String::new()
            };
            Some(format!("{bound_prefix}{suffix}"))
        } else {
            Some(direct)
        }
    }

    fn resolve_alias_binding_path(
        &self,
        alias: &Alias,
        local_bindings: &IndexMap<String, String>,
        passed_bindings: &IndexMap<String, String>,
        ...
        let path = match alias {
            Alias::WithoutPassed { parts, .. } => parts
                ...
                .join("."),
            Alias::WithPassed { extra_parts } => {
                let mut full = vec!["PASSED".to_string()];
                full.extend(extra_parts.iter().map(|part| part.as_str().to_string()));
                full.join(".")
            }
        };
        self.resolve_path_binding_path(&path, local_bindings, passed_bindings)
    ...
    fn collect_piped_call_reactive_deps(
        &self,
        from: &Spanned<Expression>,
        to: &Spanned<Expression>,
        deps: &mut Vec<String>,
        ...
        let Expression::FunctionCall { path, arguments } = &to.node else {
            return false;
        };
        let path_parts: Vec<&str> = path.iter().map(|part| part.as_str()).collect();
        match path_parts.as_slice() {
            ["List", "map"] => {
                let mut scoped_locals = local_bindings.clone();
                if let Some(item_param) = arguments
                    .iter()
                    .find(|arg| arg.node.value.is_none())
                    .map(|arg| arg.node.name.to_string())
                {
                    if let Some(source_path) =
                        self.resolve_expr_binding_path(from, local_bindings, passed_bindings)
                    {
                        scoped_locals.insert(item_param, format!("{source_path}.*"));
                    }
                }
                for arg in arguments {
                    if let Some(value) = arg.node.value.as_ref() {
    ...
    fn is_known_reactive_name(&self, name: &str) -> bool {
        self.reactive_vars.contains_key(name)
            || self
                .compiler
                .get_var_expr(name)
                .is_some_and(|expr| self.is_collection_backed_reactive_expr(expr))
    }

    fn is_collection_backed_reactive_expr(&self, expr: &Spanned<Expression>) -> bool {
        self.compiler.is_reactive(expr) && !matches!(expr.node, Expression::Object(_))
    }

    fn reactive_candidate_prefixes(&self, path: &str) -> Vec<String> {
        let mut prefixes = Vec::new();
        let mut current = path.strip_prefix("PASSED.").unwrap_or(path).to_string();
        if current.is_empty() {
            return prefixes;
        }
        loop {
            if !prefixes.iter().any(|existing| existing == &current) {
                prefixes.push(current.clone());
            }
            if current.ends_with(".*") {
                current.truncate(current.len().saturating_sub(2));
                continue;
            }
            let Some(split_idx) = current.rfind('.') else {
                break;
            };
    ...
    fn collect_sampled_reactive_deps(
        &self,
        expr: &Spanned<Expression>,
        deps: &mut Vec<String>,
        visited_functions: &mut std::collections::HashSet<String>,
        ...
        match &expr.node {
            Expression::Variable(var) => {
                self.collect_sampled_reactive_deps(&var.value, deps, visited_functions);
            }
            Expression::Alias(alias) => {
            ...
            Expression::Pipe { from, to } => {
                self.collect_sampled_reactive_deps(from, deps, visited_functions);
                self.collect_sampled_reactive_deps(to, deps, visited_functions);
            }
            Expression::When { arms } => {
                for arm in arms {
                    self.collect_sampled_reactive_deps(&arm.body, deps, visited_functions);
                }
            }
            Expression::FunctionCall { path, arguments } => {
                for arg in arguments {
                    if let Some(ref value) = arg.node.value {
                        self.collect_sampled_reactive_deps(value, deps, visited_functions);
                    }
                }
                ...
                {
                    if visited_functions.insert(qualified_name.clone()) {
                        self.collect_sampled_reactive_deps(body, deps, visited_functions);
                        visited_functions.remove(qualified_name.as_str());
                    }
                }
            }
            Expression::ArithmeticOperator(op) => match op {
                ...
                | ArithmeticOperator::Divide {
                    ...
                    self.collect_sampled_reactive_deps(operand_a, deps, visited_functions);
                    self.collect_sampled_reactive_deps(operand_b, deps, visited_functions);
                }
                ArithmeticOperator::Negate { operand } => {
                    self.collect_sampled_reactive_deps(operand, deps, visited_functions);
                }
            },
            Expression::Comparator(op) => match op {
                Comparator::Equal {
            ...
            // Nested reactive constructs like WHILE/LATEST/HOLD define their own
            // live subgraphs, so their internal deps should not be hoisted into
            // outer event-time samples. THEN is different: its body is evaluated
            // at event time and must sample the current reactive deps it reads.
            Expression::While { .. } => {}
            Expression::Then { body } => {
                self.collect_sampled_reactive_deps(body, deps, visited_functions);
            }
            ...
            Expression::Block { variables, output } => {
                ...
                self.collect_sampled_reactive_deps(output, deps, visited_functions);
            }
            Expression::Object(obj) => {
                for variable in &obj.variables {
            ...
            Expression::List { items } => {
                for item in items {
                    self.collect_sampled_reactive_deps(item, deps, visited_functions);
                }
            }
            Expression::Hold { .. } => {}
            Expression::PostfixFieldAccess { expr, .. } => {
                self.collect_sampled_reactive_deps(expr, deps, visited_functions);
            }
            _ => {}
        }
    }
...
fn get_nested_field<'a>(obj: &'a Value, path: &str) -> Option<&'a Value> {
    let mut current = obj;
    for part in path.split('.') {
...
fn decorate_reactive_scope_value_preserving_paths(value: &Value, base_path: &str) -> Value {
    match value {
        Value::Tagged { tag, fields } if tag.as_ref() == LIST_TAG => {
            let decorated_fields: BTreeMap<Arc<str>, Value> = fields
                .iter()
                .map(|(key, item)| {
                    let item_path = format!("{base_path}.{}", key.as_ref());
                    let decorated =
                        decorate_reactive_scope_value_preserving_paths(item, &item_path);
                    (key.clone(), decorated)
                })
                .collect();
            Value::Tagged {
            ...
            for (field_name, field_value) in fields.iter() {
                if field_name.ends_with("_elements") {
                    ...
                    if let Value::Object(element_fields) = field_value {
                        for (el_name, existing) in element_fields.iter() {
                            let link_path = format!("{base_path}.{}.{}", field_name, el_name);
                            let seeded = if existing.get_field(LINK_PATH_FIELD).is_some() {
                                existing.clone()
                            } else if matches!(existing, Value::Object(_)) {
                                existing
                                    .clone()
                                    .update_field(LINK_PATH_FIELD, Value::text(link_path.as_str()))
                            } else {
                ...
                let child_path = format!("{base_path}.{}", field_name);
                let decorated =
                    decorate_reactive_scope_value_preserving_paths(field_value, &child_path);
                result = result.update_field(field_name.as_ref(), decorated);
            }
...
impl Compiler {
    ...
    fn build_doc_template_inner(
        ...
        match &expr.node {
            ...
            Expression::Pipe { from: _, to } => match &to.node {
    ...
    fn expr_contains_link(&self, expr: &Spanned<Expression>) -> bool {
        match &expr.node {
            Expression::Link | Expression::LinkSetter { .. } => true,
            Expression::Variable(var) => self.expr_contains_link(&var.value),
            Expression::Pipe { from, to } => {
                self.expr_contains_link(from) || self.expr_contains_link(to)
            }
            Expression::Object(obj) => obj
                .variables
                .iter()
            ...
            Expression::ArithmeticOperator(op) => match op {
                ...
                | ArithmeticOperator::Divide {
                    operand_a,
                    operand_b,
                } => self.expr_contains_link(operand_a) || self.expr_contains_link(operand_b),
                ArithmeticOperator::Negate { operand } => self.expr_contains_link(operand),
            },
            Expression::Comparator(cmp) => match cmp {
                Comparator::Equal {
                    operand_a,
            ...
            Expression::PostfixFieldAccess { expr, .. } => self.expr_contains_link(expr),
            _ => false,
        }
    }
}
...
"#,
        ...
        assert_eq!(
            probe_display_cell_value
                .get_field(LINK_PATH_FIELD)
                .and_then(Value::as_text),
            Some("cell.cell_elements.display"),
            "A1 display cell should carry the linked display path; got {probe_display_cell_value}"
        );
        assert!(
            probe_display_cell_value
                .get_field("element")
                .and_then(|v| v.get_field(HOVER_PATH_FIELD))
                .and_then(Value::as_text)
                .is_some(),
            "A1 display cell element should carry hover path metadata; got {probe_display_cell_value}"
        );
    ...
    #[test]
    fn cells_overrides_hold_transform_preserves_marker_event_payload() {
        let source = read_example("../../playground/frontend/src/examples/cells/cells.bn");
        ...
        let overrides = graph
            .collections
            .get(&VarId::new("overrides"))
            .expect("overrides HOLD should exist");
        let CollectionSpec::HoldState { transform, .. } = overrides else {
            panic!("overrides should compile to HoldState");
        };

        let initial_state = list_value(Vec::new());
        let event = Value::object([
            ("__hold_then", Value::text("overrides")),
            (
                "__hold_event",
                Value::object([
                    ("row", Value::number(1.0)),
    ...
    #[test]
    fn cells_overrides_hold_transform_ignores_empty_marker_events() {
        let source = read_example("../../playground/frontend/src/examples/cells/cells.bn");
        let program = compile(&source, None, &std::collections::HashMap::new(), None)
            .expect("cells should compile");
        let CompiledProgram::Dataflow { graph } = program else {
        ...
        let initial_state = list_value(vec![Value::object([
            ("row", Value::number(1.0)),
            ("column", Value::number(1.0)),
            ("text", Value::text("5")),
        ])]);
        let event = Value::object([
            ("__hold_then", Value::text("overrides")),
            ("__hold_event", Value::Unit),
        ]);

        let result = transform(&initial_state, &event);
        assert_eq!(
            result, initial_state,
            "empty marker event should preserve HOLD state, got: {result:?}"
        );
    }
    ...
    #[test]
    fn cells_event_ports_compile_from_forwarded_local_events() {
        let source = read_example("../../playground/frontend/src/examples/cells/cells.bn");
        ...
        let edit_started_row = graph
            .collections
            .get(&VarId::new("event_ports.edit_started_row"))
            .unwrap_or_else(|| panic!("event_ports.edit_started_row should exist in graph"));

        assert!(
            !matches!(edit_started_row, CollectionSpec::Input(_)),
            "event_ports.edit_started_row should be driven by forwarded local events, not a raw Input"
        );
        ...
        assert!(
            !double_click_paths.is_empty(),
            "cells should register at least one double-click input path"
        );
        assert!(
            double_click_paths.iter().any(|path| {
                path == "all_row_cells.0000.cells.0000.display_element.event.double_click"
            }),
            "cells should register the nested first-cell double-click path, got {double_click_paths:?}"
        );
    }

    #[test]
    fn cells_edit_committed_forwarder_builds_commit_payload_from_keydown() {
        let source = read_example("../../playground/frontend/src/examples/cells/cells.bn");
        ...
        let public_forwarder = graph
            .collections
            .get(&VarId::new("event_ports.edit_committed"))
            .expect("event_ports.edit_committed should exist");
        let CollectionSpec::Map { source, .. } = public_forwarder else {
            panic!("event_ports.edit_committed should be a forwarded map");
        };

        let CollectionSpec::Concat(inner_sources) = graph
            .collections
            .get(source)
            .expect("event_ports.edit_committed concat should exist")
        else {
            panic!("event_ports.edit_committed should fan in per-cell forwarders");
        };
        let first_forwarder = inner_sources
            .first()
            .expect("event_ports.edit_committed should have at least one per-cell forwarder");

        let inner_forwarder = graph
            .collections
            .get(first_forwarder)
            .expect("inner edit_committed forwarder should exist");
        let CollectionSpec::Map { f, .. } = inner_forwarder else {
            panic!("inner edit_committed forwarder should be a map");
        };

        let result = f(&Value::object([
            ("key", Value::tag("Enter")),
            ("text", Value::text("7")),
        ]));
...
"#;
    ...
    fn crud_person_to_add_joins_create_press_with_text_inputs() {
        ...
        assert!(
            graph.collections.contains_key(source),
            "store.person_to_add source should exist"
        );
...
"#;
    ...
    #[test]
    fn non_keyed_list_map_preserves_nested_link_paths_in_rendered_items() {
        let source = r#"
items: LIST {
    [cell_elements: [display: LINK], text: "A"]
    [cell_elements: [display: LINK], text: "B"]
}
document: Document/new(root:
    Element/stripe(
        element: []
        ...
        gap: 0
        style: []
        items: items |> List/map(cell, new:
            Element/label(
                element: cell.cell_elements.display
                style: []
                label: cell.text
            )
        )
...
"#;
        ...
        let first_item = document_value
            .get_field("root")
            .and_then(|root| root.get_field("items"))
            .and_then(|items| items.get_field("0000"))
            .expect("first mapped item");

        assert_eq!(
            first_item
                .get_field("element")
                .and_then(|element| element.get_field(LINK_PATH_FIELD))
                .and_then(|path| path.as_text()),
            Some("items.0000.cell_elements.display"),
        );
    }

    #[test]
    fn non_keyed_list_map_preserves_top_level_link_setter_paths() {
        let source = r#"
items: LIST {
    [cell_elements: [display: LINK], text: "A"]
    [cell_elements: [display: LINK], text: "B"]
...
"#;
        ...
        assert_eq!(
            first_item
                .get_field("press_link")
                .and_then(|path| path.as_text()),
            Some("items.0000.cell_elements.display.event.press"),
        );
    }

    #[test]
    fn non_keyed_list_map_from_range_namespaces_nested_links_under_output_var() {
        let source = r#"
rows: List/range(from: 1, to: 2)
    |> List/map(row, new:
        [
            row: row
            cells: LIST {
                [cell_elements: [display: LINK], text: TEXT { {row} }]
            }
        ]
    )
document: Document/new(root:
    Element/label(element: [], style: [], label: TEXT { ok })
)
"#;
        ...
        assert_eq!(
            document_value
                .get_field("rows")
                .and_then(|rows: &Value| rows.get_field("0000"))
                .and_then(|row: &Value| row.get_field("cells"))
                .and_then(|cells: &Value| cells.get_field("0000"))
                .and_then(|cell: &Value| cell.get_field("cell_elements"))
                .and_then(|elements: &Value| elements.get_field("display"))
                .and_then(|display: &Value| display.get_field(LINK_PATH_FIELD))
                .and_then(|path: &Value| path.as_text()),
            Some("rows.0000.cells.0000.cell_elements.display"),
        );
    }
