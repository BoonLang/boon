...
// ═══════════════════════════════════════════════════════════════════════════
pub use inputs::{EventInjector, fire_global_link, fire_global_link_with_text, fire_global_link_with_bool, fire_global_blur, fire_global_key_down, clear_editing_grace_period_for_link, set_global_dispatcher, clear_global_dispatcher, set_task_handle, clear_task_handle, set_output_listener_handle, clear_output_listener_handle, set_timer_handle, clear_timer_handle, DynamicLinkAction, add_dynamic_link_action, clear_dynamic_link_actions, get_dynamic_link_action, get_all_link_mappings};
// ═══════════════════════════════════════════════════════════════════════════
...
pub use outputs::{
    ...
    OutputObserver, cell_signal, cells_signal, list_signal_vec,
    get_cell_value, get_all_cell_states, load_persisted_cell_value,
    clear_dd_persisted_states, clear_cells_memory,
    init_cell, init_cell_with_persist,
    sync_cell_from_dd, sync_cell_from_dd_with_persist, update_cell_no_persist,
    // DELETED: checkbox toggle holds functions - were set but never read (dead code)
    // Text clear cells - getter only (setter via DataflowConfig)
    is_text_clear_cell, clear_text_clear_cells,
    // Route state
    set_filter_from_route, get_current_route, init_current_route, clear_current_route,
    // Config getters - delegate to DataflowConfig via core accessors
    get_remove_event_path, get_bulk_remove_event_path,
    get_editing_event_bindings, get_toggle_event_bindings, get_global_toggle_bindings,
    // Type aliases for backward compatibility
    EditingEventBindings, ToggleEventBinding, GlobalToggleEventBinding,
    // Text input state
    set_text_input_key_down_link, get_text_input_key_down_link, clear_text_input_key_down_link,
    // List clear state
    set_list_clear_link, get_list_clear_link, clear_list_clear_link,
    // Template list flag
    set_has_template_list, get_has_template_list, clear_has_template_list,
};
