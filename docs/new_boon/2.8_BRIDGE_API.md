...
pub enum EventKind {
    Press, Click, DoubleClick, Change, KeyDown, Blur
    // Note: Hovered is NOT an event - it's a continuous state stream (see ElementState below)
}
```
...
**Note:** `hovered`, `focused`, `bounds` are **continuous state streams** (always have a value), not discrete events. See Element State Fields table below.
...
- **CLI:** No Bridge - evaluator materializes the **root value** (not the Document wrapper) to JSON
  - `document: 42 |> Document/new()` outputs `42` (not `{"root": 42}`)
  - CLI extraction: `arena.navigate_field(document_slot, "root")` → materialize that value
- `Document/new` does NOT call a renderer - that's the platform's responsibility

**Clarification for implementers:** While `Document/new` internally creates a Router with a `root` field (an "opaque handle"), the CLI specifically unwraps this to get the root value. Users never see the Router wrapper in CLI output - they see the value they passed to `Document/new`.

#### Scalar vs Element Roots

`Document/new` accepts ANY value type, not just Element:

| Root Type | Browser Behavior | CLI Behavior | Examples |
|-----------|------------------|--------------|----------|
| `Number` | Render as text content | Output as JSON number | minimal, interval, fibonacci |
| `Text` | Render as text content | Output as JSON string | hello_world |
| `Bool` | Render "True"/"False" | Output as JSON boolean | - |
| `Unit` | Render empty | Output as JSON null | - |
| `Tag` | Render tag name as text | Output as JSON `{"_type":"tag","name":"TagName"}` | - |
| `Element` | Render full UI tree | Output as JSON element tree | counter, todo_mvc |
| `TaggedObject` | Depends on tag | Output as JSON tagged object | - |
| `NoElement` | Render nothing (clear) | Output as JSON `{"_type":"tag","name":"NoElement"}` | - |

**Examples using scalar roots:**
```boon
// minimal.bn - Number root
document: 42 |> Document/new()

// interval.bn - Number root (reactive)
count: Duration[milliseconds: 1000] |> Timer/interval()
document: count |> Document/new()

// fibonacci.bn - Number root (computed)
result: 10 |> Stream/pulses() |> ...
document: result |> Document/new()

// hello_world.bn - Text root
document: TEXT { Hello, World! } |> Document/new()
```

**Why scalar roots work:** The Bridge auto-wraps scalar values into DOM text nodes. This enables simple examples without Element boilerplate.
...
### 2.8.4.1 DOM Reconciliation Approach

Boon does **NOT** implement its own virtual DOM diff algorithm. Instead, reconciliation is delegated to Zoon.

#### Why Delegation (Not Custom VirtualDOM)

| Approach | Pros | Cons |
|----------|------|------|
| **Custom VirtualDOM** | Full control | Duplicates Zoon's work, complex, bugs |
| **Delegated to Zoon** | Simple, already battle-tested | Less control (acceptable) |

**Decision: Delegate to Zoon**

#### What Bridge Tracks

The Bridge maintains a mapping between arena state and rendered elements:

```rust
pub struct Bridge {
    /// Maps arena SlotIds to rendered Zoon element handles
    element_map: HashMap<SlotId, ZoonElementHandle>,

    /// Tracks which slots were rendered last tick (for removal detection)
    last_rendered: HashSet<SlotId>,
}
```

**Per-tick reconciliation:**

1. **Element identity**: Same `SlotId` = same element (no re-creation)
2. **Attribute updates**: Forward changed properties to Zoon
3. **Child changes**: Let Zoon handle list diffing for children
4. **Removal detection**: Elements in `last_rendered` but not current → remove from DOM

```rust
impl Bridge {
    fn reconcile(&mut self, arena: &Arena) {
        let current_rendered = HashSet::new();

        // Walk arena element tree, update/create elements
        self.walk_element_tree(arena.document_root(), &mut current_rendered);

        // Remove elements that are no longer rendered
        for old_slot in self.last_rendered.difference(&current_rendered) {
            if let Some(handle) = self.element_map.remove(old_slot) {
                handle.remove_from_dom();
            }
        }

        self.last_rendered = current_rendered;
    }

    fn walk_element_tree(&mut self, slot: SlotId, current: &mut HashSet<SlotId>) {
        current.insert(slot);

        if let Some(handle) = self.element_map.get(&slot) {
            // Element exists - update properties only
            self.update_element_properties(slot, handle);
        } else {
            // New element - create it
            let handle = self.create_element(slot);
            self.element_map.insert(slot, handle);
        }

        // Recurse to children (Zoon handles child list diffing)
        for child_slot in arena.get_children(slot) {
            self.walk_element_tree(child_slot, current);
        }
    }
}
```
...
Zoon's reactive system automatically handles:

- **Child list reordering** (via its internal keying)
- **Incremental DOM updates** (only changed properties touch DOM)
- **Event listener management** (attach/detach lifecycle)

#### NoElement Handling

When a slot evaluates to `NoElement`:

1. Bridge marks slot as "should not render"
2. If slot was previously rendered, remove from DOM
3. No placeholder in DOM (truly empty)

```rust
fn walk_element_tree(&mut self, slot: SlotId, current: &mut HashSet<SlotId>) {
    let payload = self.arena.get_value(slot);

    if let Payload::Tag(tag) = payload {
        if self.arena.tag_name(*tag) == "NoElement" {
            // Don't add to current_rendered → will be removed if existed before
            return;
        }
    }
...
### 2.8.4.2 Special Tagged Objects for Elements
...
| `Hidden[text]` | `{ text: Text }` | Hidden label for accessibility | Screen readers, form labels |
...
```boon
-- Hidden provides accessible label text (not displayed visually)
Element/text_input(label: Hidden[text: TEXT { Email address }])

-- Reference for shared element access
Reference[element: shared_element]
```
- `Hidden[text]`: Accessible label for screen readers (CSS `display: none` with aria-label)
...
```rust
impl Evaluator {
    /// Normalize Duration tagged object to milliseconds
    pub fn duration_to_ms(&self, payload: &Payload, ctx: &str) -> f64 {
        let Payload::TaggedObject { tag, fields } = payload else {
            panic!("{}: expected Duration, got {:?}", ctx, payload);
        };
        let tag_name = self.arena.tag_name(*tag);
        if tag_name != "Duration" {
            panic!("{}: expected Duration, got {}", ctx, tag_name);
        ...
        let router = self.arena.get(*fields);

        // Priority: milliseconds > ms > seconds (first defined wins)
        if let Some(ms) = router.try_get_field("milliseconds") {
            return ms.expect_number(ctx);
        }
        if let Some(ms) = router.try_get_field("ms") {
            return ms.expect_number(ctx);
        }
        if let Some(secs) = router.try_get_field("seconds") {
            return secs.expect_number(ctx) * 1000.0;
        }

        panic!("{}: Duration must have seconds, ms, or milliseconds field", ctx);
    }
}

// Usage in Timer/interval:
// Duration[seconds: 1] |> Timer/interval()  →  interval of 1000ms
// Duration[ms: 500] |> Timer/interval()     →  interval of 500ms
```
