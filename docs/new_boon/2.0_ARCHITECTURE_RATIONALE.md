...
while !dirty_nodes.is_empty() {
    dirty_nodes.sort_by_key(|n| (n.source_id, n.scope_id, n.port));
...
| **FPGA target** | No mapping | Design enables HDL transpiler (§5.1) |

---

### 2.0.8 The Hardware Mental Model

The synchronous event loop design is **hardware-inspired**, not hardware-synthesizable:
...
- The arena engine (engine_v2/) is a **software** implementation for browser/CLI
- The hardware analogy guides design decisions (explicit state, deterministic ordering)
- FPGA synthesis uses a **separate HDL transpiler** (see §5.1) that compiles Boon AST → Verilog
- The same Boon **source code** can target browser (EventLoop) or hardware (clock cycles)
- The arena engine itself is NOT synthesizable - it's a runtime, not an HDL generator
