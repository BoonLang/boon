...
-- Uses tagged values to demonstrate WHEN pattern matching

items: LIST { Valid[1], Valid[2], Invalid[TEXT { bad data }], Valid[4], Valid[5] }

result: items
    |> List/map(item, new:
        item |> WHEN {
            Valid[n] => n * 2                        -- Double valid values
            Invalid[_] => FLUSH { ItemError[item] }  -- Mark invalid items as error
        }
    )

-- CLI output: [2, 4, {"ItemError": {"Invalid": "bad data"}}, 8, 10]
