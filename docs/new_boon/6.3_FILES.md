...
result: items
    |> List/map(item, new:
        item |> WHEN {
            Valid[value: n] => n * 2                        -- Double valid values
            Invalid[reason: r] => FLUSH { ItemError[source: r] }  -- Mark invalid items as error
        }
    )

-- CLI output: [2, 4, {"ItemError": {"source": "bad data"}}, 8, 10]
