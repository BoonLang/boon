...
**M0/M1 Scope:** Persistence only. HOLD values are saved to localStorage on each tick, restored on page load. Time-travel debugging requires recording event history, which is deferred.
...
// Conversion during snapshot
impl From<&Payload> for SerializablePayload {
    fn from(p: &Payload, arena: &Arena) -> SerializablePayload {
        match p {
            Payload::ListHandle(slot_id) => {
                let addr = arena.get(*slot_id).address;
                SerializablePayload::ListHandle(addr)
            }
            Payload::ObjectHandle(slot_id) => {
                let addr = arena.get(*slot_id).address;
                SerializablePayload::ObjectHandle(addr)
            }
