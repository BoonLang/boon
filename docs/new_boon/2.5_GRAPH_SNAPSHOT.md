...
pub struct GraphSnapshot {
    ...
    pub version: u32,  // Current: 1. Increment when format changes.
...
pub const SNAPSHOT_VERSION_CURRENT: u32 = 1;

impl GraphSnapshot {
    /// Check if snapshot can be restored with current code
    pub fn is_compatible(&self) -> bool {
        self.version <= SNAPSHOT_VERSION_CURRENT
    }

    /// Migrate snapshot to current version if possible
    pub fn migrate(&mut self) -> Result<(), SnapshotMigrationError> {
        match self.version {
            0 => {
                // V0→V1: Add version field (implicit upgrade)
                self.version = 1;
                Ok(())
            }
            1 => Ok(()), // Current version, no migration needed
            v => Err(SnapshotMigrationError::UnsupportedVersion(v)),
        }
    }
}
...
pub enum NodeStateSnapshot {
    ...
    Bus { items: Vec<(ItemKey, NodeAddress)>, alloc_site: AllocSiteSnapshot },  // NodeAddress
...
### 2.5.3.0.1 AllocSite Persistence

**Problem:** List items need stable identity across page reloads. If item IDs restart at 0, restored items get different ScopeIds, breaking nested HOLD state mapping.

**Solution:** Persist the `AllocSite` counter as part of Bus node snapshots:

```rust
/// AllocSite snapshot - tracks list item allocation state
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct AllocSiteSnapshot {
    /// The SourceId of the List/append call site
    pub site_source_id: SourceId,
    /// Next instance ID to allocate (monotonic, never resets)
    pub next_instance: u64,
}

impl AllocSiteSnapshot {
    pub fn from_alloc_site(site: &AllocSite) -> Self {
        Self {
            site_source_id: site.site_source_id,
            next_instance: site.next_instance,
        }
    }

    pub fn restore(&self) -> AllocSite {
        AllocSite {
            site_source_id: self.site_source_id,
...
1. Bus node is restored with `AllocSiteSnapshot`
2. `next_instance` counter continues from persisted value (e.g., 5)
3. New items get IDs 5, 6, 7... (never colliding with restored items 0-4)
4. Nested HOLDs use `ScopeId::child(item_key)` - restored items keep same ScopeId

**Why this matters:**

```boon
// todo_mvc: each item has nested HOLD for completion state
items: LIST {} |> List/append(item: new_todo())

FUNCTION new_todo() {
    [
        text: input_text,
        completed: False |> HOLD state { toggle |> THEN { !state } }
    ]
}
```

Without AllocSite persistence:
- Page reload → items restore with IDs 0, 1, 2
- But the `completed` HOLD states were persisted with original IDs (e.g., 5, 6, 7)
- State mapping fails → completion states lost

With AllocSite persistence:
- Counter restores at 8 (after original IDs 5, 6, 7)
- Restored items keep their original IDs
- Nested HOLD state maps correctly → completion states preserved
