...
| `Unit` | `[]` | Signal with no data (like Rust's `()`); runtime: `Payload::Unit`; JSON: `null` |

**Unit vs Object distinction:**
- `[]` is **Unit** - a signal/event with no data, like a button press
- `[field: value]` is **Object** - a record with fields, runtime: `Payload::ObjectHandle`
- Empty object (rare): use `[:]` syntax if truly needed (zero-field object handle)

**Unit coercion in stdlib functions:**
Examples use `element: []`, `style: []`, `placeholder: []` to mean "use defaults". Stdlib functions accept `Unit` where an options object is expected, treating it as "empty options":

```rust
// Element/button implementation accepts Unit for optional args
fn element_button(element: Payload, style: Payload, label: Payload) {
    let style_opts = match style {
        Payload::Unit => default_style(),           // [] means default
        Payload::ObjectHandle(slot) => parse_style(slot),
        _ => panic!("style: expected Object or Unit"),
    };
    // ...
}
```

This coercion is applied by stdlib functions, NOT by the type system. The type signature is `Element/button(element: Unit | Element, style: Unit | StyleObject, label: Text)` - Unit is an explicit accepted variant.
...
> **Note:** These signatures match the actual stdlib used by the 23 validation examples.
...
```
Math/sum : List<Number> -> Number
```

#### Text Functions

```
Text/empty       : Unit -> Text              // Returns ""
Text/is_not_empty: Text -> Bool
Text/space       : Unit -> Text              // Returns " "
Text/trim        : Text -> Text
```
...
```
List/append  : (item: T) -> ListOp<T>        // Append item to list
List/remove  : (item: T) -> ListOp<T>        // Remove item from list
List/retain  : (predicate: T -> Bool) -> ListOp<T>  // Keep matching items
List/clear   : Unit -> ListOp<T>             // Remove all items
List/count   : List<T> -> Number
List/is_empty: List<T> -> Bool
List/map     : (List<T>, (item: T, new: T -> U)) -> List<U>
```

**Note:** List operations are applied via pipe: `LIST {} |> List/append(item: value)`
...
```
Log/info : (message: Text) -> Unit           // Console log (effect)
```

#### Router Functions

```
Router/go_to : (path: Text) -> Unit          // Navigate to path (effect)
Router/route : Unit -> Text                  // Current route path
```

#### Document Functions

```
Document/new : (root: Element | Text | Number | Object | List) -> Document
```

**Document return type:** `Document` is an opaque handle (internally a Router with a `root` field). It is NOT the root value itself.

**CLI output behavior:** When printing a `document` binding, CLI materializes the **root value** (not the Document wrapper). This means:
- `document: 42 |> Document/new()` outputs `42` (not `{"root": 42}`)
- `document: TEXT { Hello } |> Document/new()` outputs `"Hello"`
- See §CLI Semantics in the implementation plan for full details.

**Scalar roots:** When root is Text or Number, it renders as text content. Element roots render as full UI trees.

#### Element Functions

```
Element/button     : (element: Unit | Element, style: Unit | Style, label: Text) -> Element
Element/checkbox   : (element: Unit | Element, style: Unit | Style, label: Hidden[text: Text] | Reference[element: Element], checked: Bool, icon: Element) -> Element
Element/container  : (element: Unit | Element, style: Unit | Style, items: List<Element>) -> Element
Element/label      : (element: Unit | Element, style: Unit | Style, label: Text) -> Element
Element/link       : (element: Unit | Element, style: Unit | Style, to: Text, items: List<Element>) -> Element
Element/paragraph  : (element: Unit | Element, style: Unit | Style, content: Text) -> Element
Element/stack      : (element: Unit | Element, style: Unit | Style, items: List<Element>) -> Element
Element/stripe     : (element: Unit | Element, style: Unit | Style, items: List<Element>, direction: Row | Column, gap: Number) -> Element
Element/text_input : (element: Unit | Element, style: Unit | Style, label: Hidden[text: Text], text: Text, placeholder: [text: Text], focus: Bool) -> Element
```

**Unit coercion:** All Element functions accept `Unit` (written as `[]`) for optional parameters. `element: []` means "use defaults", `style: []` means "no custom styling".

#### Built-in Tagged Objects

```
Duration[seconds: Number]       // Time duration (converts to milliseconds)
Duration[milliseconds: Number]
Duration[ms: Number]            // Alias for milliseconds

Oklch[lightness: Number, chroma: Number, hue: Number]  // Color in Oklch color space
Oklch[lightness: Number, chroma: Number, hue: Number, alpha: Number]  // With alpha

Hidden[text: Text]              // Hidden text (accessibility: read but not displayed)

Reference[element: Element]     // Reference wrapper for element state

NoElement                       // Tag-only value: renders nothing (conditional hide)
```

**NoElement:** Unlike other tagged objects, `NoElement` is a tag with no fields. It's used to conditionally hide elements:

```boon
show_greeting |> WHILE {
    True => Element/label(label: TEXT { Hello })
    False => NoElement
}
```
