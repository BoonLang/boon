...
| `Bool` | `True`, `False` | Represented as tags |
| `Unit` | `[]` | Empty object, often used for signals |
...
```boon
todo: Todo[text: "Buy groceries", done: False]
...
Errors are represented as **regular union types** (not a special `Flushed<T>` wrapper):
...
```
Stream/interval : Number -> Number           // Emits tick count
Stream/pulses   : Number -> Number           // Emits 0..n-1 across n ticks
Stream/delay    : (T, Number) -> T           // Delays value by ms
Stream/throttle : (T, Number) -> T           // Rate limit
Stream/debounce : (T, Number) -> T           // Debounce
```
