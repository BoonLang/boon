...
```boon
// Old combined state
app_state: [users: LIST {...}, settings: [...]] |> HOLD state { ... } |> DRAINING

// Split into separate variables
users: DRAIN { app_state.users } |> HOLD state { ... }
settings: DRAIN { app_state.settings } |> HOLD state { ... }
```

**Semantics:**
- Drains specific field from object
- Parent object still exists but field is empty/removed
- Nested HOLDs within that field are preserved
...
app: [users: LIST {...}, config: [...]] |> HOLD state { ... }

// Step 1: Mark app as DRAINING, add field-level DRAINs
app: [users: LIST {...}, config: [...]] |> HOLD state { ... } |> DRAINING
users: DRAIN { app.users } |> HOLD state { ... }
config: DRAIN { app.config } |> HOLD state { ... }

// Step 2: Remove all markers
users: LIST {...} |> HOLD state { ... }
config: [...] |> HOLD state { ... }
```
...
```boon
// module_a.bn
users: LIST { ... } |> HOLD state { ... } |> DRAINING

// module_b.bn (imports module_a)
import module_a

// DRAIN from imported module
local_users: DRAIN { module_a.users } |> HOLD state { ... }
```
...
```boon
// Backend (server.bn)
users: LIST { User[...] } |> HOLD state { ... } |> DRAINING

// Frontend (app.bn)
import server

local_users: DRAIN { server.users } |> HOLD state { ... }
```
