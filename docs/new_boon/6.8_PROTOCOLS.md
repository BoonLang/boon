...
- `Unit` in Boon is written as `[]` (empty object literal) and becomes `Payload::Unit` at runtime
...
```json
{
  "source_id": 42,     // SourceId from parser
  "scope_id": 123,     // Hashed scope path
  "port": 0            // For multi-output nodes (default 0)
}
```

**Shorthand format for debugging:** `"42:123:0"` (source:scope:port)
...
{
  "_type": "stream",
  "address": {"source_id": 42, "scope_id": 123, "port": 0},
  "current_value": 5
}
```
...
**Note:** Examples below use shorthand `"42:123:0"` format (source:scope:port) for node IDs.
Production implementation should use full NodeAddress objects for consistency.
