...
impl Payload {
    pub fn to_protocol_json(&self) -> serde_json::Value {
        match self {
            ...
            Payload::Tag(id) => json!({
                "_type": "tagged",
                "tag": INTERN_TABLE.get_name(*id),
                "value": /* serialize inner value */
            }),
            // ...
        }
    }
...
**Note:** Both `Unit` and `Nothing` serialize to `null`. They differ semantically:
- `Unit` = intentional "signal" value (like Rust's `()`)
- `Nothing` = absence of value (like `None`)
