...
### New Engine (to be created in shared `engine/`)
The new engine lives in `src/engine/` (not `platform/browser/`) to be platform-agnostic. It uses platform traits from M0.

- `engine/mod.rs` - Arena, SlotId, ReactiveNode
- `engine/event_loop.rs` - EventLoop, timer queue
- `engine/message.rs` - Message, Payload, routing
- `engine/nodes.rs` - Node kinds (Router, Register, Transformer, etc.)
- `evaluator.rs` - Evaluator using SlotId (shared)
- `platform/browser/bridge.rs` - Browser-specific rendering (uses platform traits)
- `platform/browser/api.rs` - Browser-specific API implementations
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   engine/                 ← NEW: Platform-agnostic engine       │
│   ├── arena.rs                (shared by CLI and browser)       │
│   ├── event_loop.rs                                             │
│   ├── message.rs                                                │
│   └── nodes.rs                                                  │
│                                                                 │
│   evaluator.rs            ← Shared evaluator using SlotId       │
│                                                                 │
│   platform/                                                     │
│   ├── traits.rs           ← Platform abstraction (from M0)      │
│   ├── browser/                                                  │
│   │   ├── bridge.rs       ← Browser rendering (Zoon)            │
│   │   └── api.rs          ← Browser API implementations         │
│   └── cli/                                                      │
