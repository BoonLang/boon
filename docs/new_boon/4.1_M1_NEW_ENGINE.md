...
- **Requires**: M0 (Boon CLI) - platform abstraction traits and shared engine foundation
- **Enables**: M2 (Language Server), M3 (FPGA Transpiler), M5 (Multi-File Modules), M9 (Cross-Platform Testing)

---

## Scope Clarification: Runtime Only

**M1 is a runtime engine milestone, NOT a type system milestone.**

| Included in M1 | NOT included in M1 |
|----------------|---------------------|
| Arena-based node allocation | Type inference |
| Message routing and propagation | Type checking |
| LATEST, HOLD, THEN, WHEN, WHILE | Type annotations in source |
| List operations | Type error reporting |
| Timer queue and events | LSP integration |
| Bridge to Zoon UI | Editor integration |

**Why no types in M1:**
- Type system is orthogonal to runtime correctness
- M1 uses dynamic typing (Payload can hold any value)
- Type inference is a separate concern (M2)
- Runtime works without types (like JavaScript, Python)

**M2 builds the type system** using M1's runtime semantics as a reference. See `4.2_M2_LANGUAGE_SERVER.md` for details on the M1→M2 relationship.
...
### New Engine (to be created in `engine_v2/`)

> **Canonical plan:** See `plans/M0_CLI__M1_NEW_ENGINE_IN_PLAYGROUND.md` for detailed implementation.

The new engine lives in `src/engine_v2/` (not `platform/browser/`) to be platform-agnostic. It uses platform traits from M0.

- `engine_v2/mod.rs` - Re-exports
- `engine_v2/arena.rs` - Arena, SlotId, ReactiveNode
- `engine_v2/event_loop.rs` - EventLoop, timer queue
- `engine_v2/message.rs` - Message, Payload, routing
- `engine_v2/node.rs` - Node kinds (Router, Register, Transformer, etc.)
- `evaluator_v2/mod.rs` - Evaluator using SlotId
- `evaluator_v2/api.rs` - API function bindings
- `platform/browser/bridge_v2.rs` - Browser-specific rendering (Zoon)
- `platform/cli/mod.rs` - CLI-specific runtime
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   engine_v2/              ← NEW: Platform-agnostic engine       │
│   ├── arena.rs                (shared by CLI and browser)       │
│   ├── event_loop.rs                                             │
│   ├── message.rs                                                │
│   └── node.rs                                                   │
│                                                                 │
│   evaluator_v2/           ← NEW: AST → arena compilation        │
│   ├── mod.rs                                                    │
│   └── api.rs                                                    │
│                                                                 │
│   platform/                                                     │
│   ├── traits.rs           ← Platform abstraction (from M0)      │
│   ├── browser/                                                  │
│   │   ├── bridge_v2.rs    ← Browser rendering (Zoon)            │
│   │   └── ...             ← OLD engine (kept during migration)  │
│   └── cli/                                                      │
│       └── ...             ← CLI implementations (M0)            │
│                                                                 │
│   Feature flag: USE_NEW_ENGINE (during transition)              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
- `Stream/pulses` - Emit N sequential values from count
...
- `fibonacci.bn` - Stream/pulses, Stream/skip

**Exit criteria:**
- `interval.bn` counts up automatically
- Timer cancellation works
- Stream/pulses emits correct sequence
- LINK events propagate correctly

### Phase 7: Bridge & Full UI

**Files to create/modify:**
```
bridge_v2.rs                # NEW: Arena-based rendering
evaluator_v2/api.rs         # API functions (canonical location per plan)
                            # Contains: Element/*, Document/*, List/*, Math/*, Text/*, Router/*
```

**Note:** API functions live in `evaluator_v2/api.rs` (single file) rather than separate `api_v2/` directory.
This matches the canonical M0/M1 plan. Functions are organized by dispatch table, not by file.
...
4. **New engine is default:**
   - [ ] Feature flag defaults to new engine
   - [ ] Old engine kept for fallback (deletion is post-M1 cleanup)
...
## M.7 Critical Files Summary

> **Canonical list:** See `plans/M0_CLI__M1_NEW_ENGINE_IN_PLAYGROUND.md` for complete file list.

| File | Status | Purpose |
|------|--------|---------|
| `src/engine_v2/mod.rs` | NEW | Re-exports |
| `src/engine_v2/arena.rs` | NEW | Arena, SlotId, ReactiveNode |
| `src/engine_v2/event_loop.rs` | NEW | EventLoop, timer queue |
| `src/engine_v2/message.rs` | NEW | Message, Payload, routing |
| `src/engine_v2/node.rs` | NEW | Node kinds |
| `src/evaluator_v2/mod.rs` | NEW | AST → arena compilation |
| `src/evaluator_v2/api.rs` | NEW | API function bindings |
| `src/platform/browser/bridge_v2.rs` | NEW | Arena → Zoon |
| `src/platform/cli/mod.rs` | NEW | CLI runtime |
