...
| **Add List/map to pipe** | Items preserve identity via pid_map | `todos \|> List/map(item, new: styled(item))` |
...
| **Change interval/timeout** | Stateless - new value applies | `1000 \|> Stream/interval()` -> `500` |
