...
**WASM integration:** Reactive propagation uses `queueMicrotask`, timer scheduling uses `setTimeout`. Note: `requestAnimationFrame` is NOT used for scheduling (see §2.4.12 for rationale).
...
**Status: RESOLVED** - All 23 playground examples are now analyzed in 6.2_EXAMPLES.md.

~~**CRITICAL FIX:** 6.2_EXAMPLES.md line 508 uses `item_index` for scoping~~ ✅ FIXED

**Resolution summary:**
- 6.2_EXAMPLES.md now covers all 23 examples from playground/frontend/src/examples/
- The `item_index` bug was corrected
- Outdated examples (latest.bn, then.bn, when.bn, while.bn) were removed from doc
...
**Correction:** Unify to single definition matching §2.3.1:
...
pub enum Payload {
    ...
    Bool(bool),         // True/False (native bool for efficiency)
    Unit,               // Signal with no data (like Rust's `()`)

    // Containers (handles, not storage)
    ListHandle(SlotId),      // Reference to Bus node
    ObjectHandle(SlotId),    // Reference to Router node
    TaggedObject {           // e.g., Duration[seconds: 1], Hidden[text: ...]
        tag: u32,            // Tag ID (Duration, Oklch, Hidden, Reference)
...
**Note:** `TaggedObject` is REQUIRED for the 23 validation examples (Duration, Oklch, Hidden, Reference).
...
**FLUSH boundary:** When FLUSH hits variable assignment within BLOCK, it unwraps at that binding (per §2.6.4).
    ...
    pub fn run_tick(&mut self) {
        ...
        while !self.dirty_nodes.is_empty() {
            ...
            self.dirty_nodes.sort_by_key(|s| {
                ...
                (node.source_id, node.scope_id, node.port)
            });
...
pub struct NodeSnapshot {
    ...
    pub current_value: Option<SerializablePayload>,  // Handles → NodeAddress
    pub version: u64,
    pub dirty: bool,
...
pub enum NodeStateSnapshot {
    Producer,
    Wire,
    Router { fields: HashMap<FieldId, NodeAddress> },  // NodeAddress (NOT SlotId - unstable across reload)
    Bus { items: Vec<(ItemKey, NodeAddress)>, alloc_site: AllocSite },  // NodeAddress
    Register { stored_value: SerializablePayload },  // Handles converted to NodeAddress
    Combiner { last_input: Option<u8> },
    Transformer,
    SwitchedWire { current_arm: Option<usize> },
    LinkNode { bindings: Vec<LinkBindingSnapshot> },
    Timer { deadline_tick: u64, deadline_ms: f64 },  // Both for deterministic restore
    TextTemplate { template: String, cached: Option<String> },
    Effect { last_execution_tick: u64 },
}

// NOTE: SlotId vs NodeAddress in snapshots:
// - SlotId = local arena reference (index + generation), NOT serializable
...
pub fn compute_stable_source_id(node: &AstNode, parent_path: &[u32]) -> u64 {
    ...
    // IMPORTANT: Use stable hash - DefaultHasher is NOT stable across Rust versions!
...
- Browser schedules propagation via `queueMicrotask`, timer events via `setTimeout` (see §2.4.12)
...
**Browser integration (see §2.4.12 for rationale):**
```rust
// Reactive propagation uses microtasks (immediate, no delay)
queue_microtask(|| event_loop.run_tick());

// Timer scheduling uses setTimeout (throttled in background tabs)
set_timeout(|| event_loop.process_timers(), timer_interval_ms);

// Note: requestAnimationFrame is NOT used for reactive scheduling
// (introduces 16ms latency). It may be used for coordinating animations
// or skipping renders when backgrounded, but not for tick scheduling.
```
