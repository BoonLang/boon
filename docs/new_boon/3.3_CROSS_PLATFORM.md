...
**Browser-Only (DOM Rendering) - ~1,500 LOC:**
- Element/* (creates DOM elements - CLI uses JSON representation)
- Router/*, Scene/*, Theme/* (browser navigation/styling)

**Note:** M0 CLI and M1 playground both use Document/new. In CLI, the element tree serializes to JSON.
In browser, it renders to DOM. Element functions create a portable data structure that the bridge
interprets per-platform.
