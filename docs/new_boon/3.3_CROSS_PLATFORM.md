...
impl PlatformTimer for TokioTimer {
    ...
    fn schedule_microtask(&self, callback: Box<dyn FnOnce() + 'static>) {
        ...
        //
        // REENTRANCY GUARD: EventLoop.tick_scheduled flag (see §2.4.12)
        // - mark_dirty() calls schedule_tick() which sets tick_scheduled=true
        // - If already true (in tick), schedule_tick() returns early (no-op)
        // - tick_scheduled resets to false at START of run_tick()
        // - Events during tick will schedule another tick for AFTER current finishes
        //
        // This ensures no nested run_tick() calls while allowing events during
        // tick processing to be handled in a subsequent tick.
        callback();
    }
