...
impl PlatformTimer for TokioTimer {
    ...
    fn schedule_microtask(&self, callback: Box<dyn FnOnce() + 'static>) {
        // CLI: run immediately (no paint cycle)
        //
        // REENTRANCY WARNING: Running callback() synchronously here can cause
        // nested run_tick() calls if mark_dirty() is called during tick processing.
        // Solutions:
        // 1. Use tokio::task::spawn_local() to defer to next poll
        // 2. Add in_tick guard in EventLoop that queues callbacks during tick
        // 3. Document that CLI tick processing is single-threaded and queue-based
        //
        // For M0/M1, option 2 (tick_scheduled flag) is already in EventLoop.
        callback();
    }
