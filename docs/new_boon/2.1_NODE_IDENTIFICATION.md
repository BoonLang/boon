...
**SourceId Hash Rule - What IS and ISN'T Hashed:**

| Hashed (affects SourceId) | NOT Hashed (survives changes) |
|---------------------------|-------------------------------|
| Node kind (Variable, FunctionCall, etc.) | Whitespace (spaces, tabs, newlines) |
| Parent path (ancestor nodes) | Comments (`-- ...`) |
| Variable/function names | Parentheses for grouping |
| Field names | Line/column positions |
| Literal values (numbers, text content) | Span information |
| Operator symbols (+, -, |>, etc.) | Formatting/indentation |
...
impl SourceId {
    ...
    pub fn compute(node: &AstNode, parent_hash: u64) -> Self {
        use std::hash::{Hash, Hasher};
        use siphasher::sip::SipHasher13;

        // Fixed seed for determinism across builds
        let mut hasher = SipHasher13::new_with_keys(0xDEADBEEF, 0xCAFEBABE);

        // Hash parent path (for uniqueness within program)
        parent_hash.hash(&mut hasher);

        // Hash semantic content ONLY:
        std::mem::discriminant(&node.kind).hash(&mut hasher);  // Node kind
        match &node.kind {
            AstKind::Variable { name, .. } => name.hash(&mut hasher),
            AstKind::Literal { value, .. } => value.hash(&mut hasher),
            AstKind::FieldAccess { field, .. } => field.hash(&mut hasher),
            // ... other node kinds
        }

        Self {
            stable_id: hasher.finish(),
            parse_order: node.parse_order,
        }
    }
}
```
...
**Why this matters:**
- Reformatting code (adding whitespace, comments) doesn't invalidate persistence
- Moving code to a different line doesn't break hot reload
- Same source structure → same SourceId → stable node identity
...
// - FieldId is an interned string index (u32) for O(1) lookup - NOT a hash
// - Example: store: [items: LIST {}, filter: Text] has ports Field(intern("items")), Field(intern("filter"))
// - Protocol JSON uses string field names; engine uses FieldId indices
```
