...
### 3.4.2 Test Platform Components
...
### 3.4.5 Mock Renderer (Output Capture)
...
### 3.4.8 Test Macros for cargo test
...
boon_test!(counter, "examples/counter/counter.bn", |test| {
    ...
    test.trigger_event("button.event.press", Payload::Unit);
    test.tick();
...
"#, |test| {
    test.run();
    assert_boon_output!(test, "0");
    test.trigger_event("click", Payload::Unit);
    test.tick();
    assert_boon_output!(test, "1");
});
```
...
"#, |test| {
    test.run();
    test.trigger_event("inc", Payload::Unit);
    test.trigger_event("inc", Payload::Unit);
    test.tick();
    assert_boon_output!(test, "2");
