## Chapter 3.1: Modules & Filesystem
...
### 3.1.1 Virtual Filesystem (Arena-Native)
...
impl Interpreter {
    pub fn run_project(
        ...
        if self.virtual_fs.exists("BUILD.bn") {
            ...
            //
            // PLATFORM NOTE (see §2.4 Issue 28):
            // - CLI/Server: Can use blocking run_until_quiescent
            // - Browser: Must use async processing before first render
            //   Browser schedules BUILD.bn evaluation, then pumps ticks
            //   via microtasks until quiescent, THEN evaluates main file.
...
### 3.1.4 Multi-Renderer Architecture
...
### 3.1.5 Scene API for 3D Rendering
...
### 3.1.7 todo_mvc_physical Compatibility
