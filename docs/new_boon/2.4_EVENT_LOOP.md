...
impl EventLoop {
    pub fn run_tick(&mut self) {
        ...
        while !self.dirty_nodes.is_empty() {
            ...
            // This ensures reproducible execution order across runs
...
- **CLI difference:** In CLI (§3.3.6), `schedule_microtask` may run synchronously. The flag prevents infinite reentrancy: tick schedules tick schedules tick...
